...
- Keep the complete XY footprint inside `+/-0.90` source tiles for the existing
  two-by-two scenery footprint.
- Build a low twelve-piece sunken plinth around a filled recessed core, then
  arrange six asymmetric supports around it. Three supports bend inward into a
  deliberately incomplete crown; the other three terminate as shorter shards.
...
```bash
python3 scripts/generate-void-rift-source.py
python3 scripts/generate-void-rift-source.py --check
```

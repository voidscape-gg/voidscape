...
| `openrsc_classic_combat_baseline` | `false` / omitted | `false` / omitted | `false` / omitted | Server-only comparison switch for the inherited OpenRSC classic formula. Keep this and both OSRS formula flags off while Overmatch is enabled. |
| `overmatch_combat` | `true` | `true` | `true` | Official launch opposed-margin combat ruleset. Mutually exclusive with OpenRSC/OSRS formula flags; stake-duel replay proofs remain unavailable under this ruleset. |

...
| `overmatch_combat` | `true` only for an approved local Overmatch trial | `false` / omitted | `false` | Server-only opposed-margin combat option. Mutually exclusive with OpenRSC/OSRS formula flags; proof-ineligible and never enabled by the launch contract. |

...
### 2026-07-09 - Android saved logins are explicit and Keystore-encrypted

Native Android now stores the selected login and saved-account roster as one versioned AES-256-GCM envelope under `noBackupFilesDir`, using a non-exportable Android Keystore key, fresh IV/AAD, `AtomicFile` commit, reopen/decrypt verification, and an authoritative tombstone for forget/reset. Legacy `credentials.txt` and `accounts.txt` migrate only when no encrypted state or tombstone exists and are deleted only after verified commit; corrupt ciphertext or a missing key fails closed without plaintext fallback. Fresh logins are never saved implicitly: Remember is an opt-in applied only after successful authentication, wrong-password attempts persist nothing, saved accounts can be forgotten while logged in, and unavailable storage offers an explicit reset path. Backup and device-transfer rules exclude app data defensively. Emulator self-test covered migration, Unicode/comma secrets, ciphertext freshness, corruption, tombstone, forget, and clear; the player-facing smoke covers wrong-password non-persistence, successful encrypted save, restart prefill, opt-out forget, and empty restart under `tmp/android-apk-polish/credentials-encrypted-final-2`. Files touched: shared credential-store contract/models, native Keystore adapter and Activity bridge, login/account flows, Android backup rules, debug clear hook, and smoke QA; no packet, opcode, server rule, DB schema, cache asset, or client version changed.

### 2026-07-09 - Successful Android login dismisses the soft keyboard

A successful first login now explicitly closes the native Android IME before entering the world, while background reconnects remain neutral so an in-game chat keyboard is not dismissed unnecessarily. This closes an intermittent resume/logout obstruction where a login IME could survive world entry and intercept a bottom-edge settings tap. The final credential smoke verifies the keyboard stays out of the reconnect/logout path. Shared login UI only; no packet, opcode, server rule, DB schema, cache asset, or client version changed.

### 2026-07-09 - Direct Android APK promotion requires signed, clean provenance

The canonical Android build now embeds schema-v3 provenance inside the signed APK and mirrors it in the sidecar: the full Git commit, a deterministic digest of Android/shared-client/cache/build inputs, clean-input state, and any explicit dirty-release override. Release builds reject dirty inputs by default and refuse metadata if those inputs change while Gradle runs; staging endpoint rewrites opt into a visibly non-promotable dirty artifact. The APK promotion checker requires one explicitly trusted signer, valid signature and non-debuggable manifest, matching package/version/SDK/client-server values, matching bytes/hash/size, a clean checkout at the exact embedded commit and digest, and byte-for-byte agreement between signed provenance and sidecar, closing the forged-sidecar gap. Regression coverage includes a passing signed release-shaped fixture plus debug, malformed metadata, forged digest, forged commit, wrong signer, and dirty checkout rejection. The portal no longer guesses a local APK path: `PORTAL_ANDROID_APK` must point at the exact promoted artifact. Direct APK promotion only; the separate Play-bundle checker remains outside this change.

### 2026-07-09 - Android bundled bootstrap is atomic and repeat-launch efficient

The Android wrapper now identifies a completed bundled-cache install by APK version plus install time, verifies every startup-critical archive is nonempty, skips unchanged assets on ordinary launches, and repairs a missing or truncated required archive. Installs write and fsync same-directory temporary files before atomic rename, clear the completion marker on interruption/failure, expose player-facing Retry/Close recovery, and no longer relaunch after a destroyed splash Activity. Server host and port are committed as one canonical preference transaction with atomically repaired compatibility mirrors; incomplete debug overrides are ignored and release builds reset a developer endpoint as a pair. A focused offline smoke covers injected mid-install failure, relaunch recovery, marker skip, required-file repair, and endpoint mirror repair under `tmp/android-apk-polish/bootstrap-atomic-final`. Remote cache updating remains disabled; this changes Android bootstrap/storage only, with no cache asset bytes, packet, opcode, gameplay rule, DB schema, or client version change.

### 2026-07-09 - Android connection and audio resources have bounded lifecycles

Shared TCP streams now close the superseded socket whenever login/reconnect publishes a replacement, and graceful logout drains its confirmation packet before closing; the socket writer is named, race-safe, and exits on replacement, logout, interruption, or setup/write failure. Native Android tracks only validated connectivity under one synchronized map, holds external-link Activities weakly, and owns sound effects in an asynchronous eight-player pool that releases on completion, error, background, logout, and Activity destruction. Focused Android smoke proved real app-switch reconnect, zero writer threads after logout, prayer sound start/completion, background audio suspension/resume, normal logout, and a usable post-logout keyboard under `tmp/android-apk-polish/network-audio-magic-final-2` and `network-audio-lifecycle-final-2`. Files touched: shared network ownership and logout paths, native `GameActivity`, Android link/audio wrappers, and `scripts/android-smoke.sh`; no opcode, packet shape, gameplay rule, server timeout, DB schema, cache asset, or client version changed.

### 2026-07-09 - Android touch, Back, and login recovery polish

Native Android input now dispatches hardware/synthetic special keys once per press, uses the view queue instead of allocating timer threads for key-release pulses, and routes a swipe wholly to an open panel or scrollable message area so scrolling UI cannot also rotate or pitch the world camera. Android Back closes a context menu before clearing an active spell/item selection while retaining the cast-last-spell memory. Login field taps reliably reopen the soft keyboard after an error, every canvas Cancel path dismisses it, and connection-failure copy allows an immediate retry. The Android smoke harness now proves same-form password-error recovery, keyboard dismissal, Back selection clearing and reselection, map key isolation, camera rotation, panel-scroll camera isolation, and one-finger no-zoom behavior; focused evidence is under `tmp/android-apk-polish/login-touch-final`, `back-selection-final`, `world-map-input`, `camera-routing-final-3`, and `zoom-one-finger-final-2`. Files touched: native `InputImpl.java`, shared `mudclient.java`, and `scripts/android-smoke.sh`; no packet, opcode, server rule, DB schema, cache asset, or shared client version changed. A real two-finger pinch remains a physical-device release check because ADB cannot synthesize it reliably.
...
### 2026-07-06 - Combat retune for early PvM friction and PvP high rolls

Adjusted the active classic combat formula so low-level NPCs can chip new players again and PvP produces high melee follow-ups slightly more often. NPC melee against players now uses an effective Attack/Strength floor of 15, with higher NPCs gaining `+1` effective Attack/Strength per 8 levels from level 40 upward, capped at `+12`; player outgoing PvM damage is unchanged. PvP melee momentum now triggers at 68% of target-adjusted max hit instead of 75%, while keeping the one-stack, target-bound, miss-cleared rule. The combat simulator mirrors the formula and corrects the built-in starter scenarios to 10 Hits. Files: `CombatFormula.java`, `tools/combat-sim/`, combat docs. No packet, opcode, DB schema, client-version, cache asset, item definition, NPC definition, or combat cadence changed; reversibility is restoring the old constants/helper logic and simulator ruleset.

### 2026-07-06 - AFK-training tuning: idle tiers, wider aggro, denser wilderness

Tuned Voidscape's AFK melee-training baseline while keeping the behavior config-driven. Regular players now use a 10-minute movement-idle warning window and active Void subscribers use a 15-minute window, with subscriber timeout falling back to the regular value when omitted; combat still does not reset movement idle, preserving wilderness PK risk. Voidscape `local.conf` raises the default aggro scan radius to 4 tiles while the Java default remains 1 for authentic presets, and the old Bandit/Undead One special cases no longer narrow the configured default; Black Knights keep their 10-tile override. A new `wilderness_spawn_multiplier` config duplicates attackable wilderness NPC locs at boot, jittering copy starts inside existing roam bounds so the authentic distribution gets denser without hand-authored spawn files; non-attackable wilderness service/quest NPCs are skipped, and the world NPC list capacity is raised from 4,000 to 5,000 so the original 2x density boots under the custom client's 5,000 server-index ceiling. Files: `ServerConfiguration`, `GameStateUpdater`, `NpcBehavior`, `WorldPopulator`, `World`, `server/local.conf`, and config/tick-loop docs. No packet, opcode, schema, client version, cache asset, XP multiplier, or spawn JSON format changed; reversibility is restoring the local config values, removing `idle_timer_subscriber` / `wilderness_spawn_multiplier` reads and the NPC-capacity bump, and reinstating the old per-NPC aggro-radius cases.
...
### 2026-07-05 - Player title overhaul: tiered catalog, contested uniques, overhead-only display

Replaced the old scope/rarity title catalog with the prelaunch tier model from `docs/TITLE-OVERHAUL-BRIEF.md`: 54 explicit titles across Renown, Feat, and Unique, with unique lifecycles for first-claim, contested, and item-bound titles. The old one-unique-per-account / `pt_b_*` block mechanism and chat-prefix injection are gone; login migration wipes obsolete title cache state while preserving `founder`. New hooks award title progress from skills, quests, wilderness PKs, NPC kills/drops, smithing, gnomeball, Void content, Void Arena seasons, and auction-house sales; `RIFTBOUND` remains staff/manual until a real Void Rift win event exists. The `::titles` UI and staff `::granttitle`/`::revoketitle` commands now speak tiers/lifecycles instead of rarity/scope. Client protocol bumps to `10123`: custom player appearance sends a title-tier byte after the active title string, the client colors overhead suffixes by tier and hides titles while overhead chat is active, and normal public chat no longer receives title prefixes. Reversibility is restoring the previous `PlayerTitle` enum/command filters/chat prefix path, downgrading the appearance packet/client version, and dropping the new hook calls plus auction seller-volume query.

### 2026-07-05 - Launch traversal markers and unreleased Void access lockdown

Added an in-minimap X marker for the last minimap walk click, drawn in the existing clipped minimap pass so players get immediate feedback without changing walk packets. Public release traversal now keeps ordinary Void Rifts limited to the approved city/Enclave network and adds a dedicated Void Arena rift in the Void Enclave; direct `::arena enter` no longer teleports players and points them back to the rift. Unreleased Void endpoints are closed for launch: Void Colossus and Void Dungeon gates are off in local config and their entry handlers deny stale/spawned objects, the Void Knight ladder/challenge is sealed while preserving the return ladder, and the Void Herald no longer starts Void Rush or Undead Siege. Files: `mudclient.java`, `VoidRift.java`, `VoidArena.java`, `VoidDungeon.java`, `VoidColossusArena.java`, `DeathMatchArena.java`, `VoidHerald.java`, `SceneryLocsVoidRift.json`, launch config/docs. No opcode, packet shape, schema, client-version, or cache-asset change; reversibility is re-enabling the content gates/menu entries and removing the minimap marker fields/draw hook.

### 2026-07-05 - Hit-splat streak visual removed

Simplified the tiered hit-splat presentation back to two visuals: the default splat for normal/scripted damage and the gold high splat for hits at least 75% of the attacker's max hit. The streak splat/tracking and the dead HUGE tier were removed from the shared client, `hit-splat-streak.png` was dropped from the source cache, and the high splat keeps the larger centered art footprint while using identical centered number rendering to the default splat. The type-10 damage metadata protocol, packet version, combat rolls, and server behavior are unchanged. Reversibility is restoring the streak asset/tracking and the streak asset selection path.

### 2026-07-05 - Starter path kits use shared Classic basics

Rebalanced the Void Island starter path kits so the path choice is an XP identity rather than a hard class lock. All paths now top up a shared Classic-style kit (basic tools, bronze long sword/shield, one extra cooked meat, and tutorial runes) without duplicating one-per-player basics that are already in inventory/equipment/bank. Warrior now adds only an iron short sword and 5 extra cooked meat; Forager adds fishing rod, 25 bait, hammer, and 50 coins; Arcanist adds extra air/mind/fire runes, shortbow, 50 bronze arrows, and a blue wizard hat. This removes the old Warrior bronze armour/iron 2h loadout, lowers Forager cash/bait, and drops Arcanist's robe/food pile while keeping every starter able to fish, mine, woodcut, fight, and cast basic spells. Files: `VoidPath` plus starter-flow/beta docs. No packet, opcode, schema, client-version, or cache-asset change; old accounts keep existing inventories and path state.

### 2026-07-05 - Traversal rift network and saved world-map walks

Reopened ordinary Void Rift travel as a small hub network instead of the old broad city teleport menu: hub rifts now route between Void Enclave, Edgeville, Varrock, Falador, and Ardougne, with Lumbridge intentionally absent so the level-0 Home teleport remains the Lumbridge path. The starter-area rift at `(139,636)` stays as an access rift into the same five destination choices, and feature rifts for Void Dungeon/Colossus remain isolated by coordinate-specific handlers. The shared world-map panel now has five client-local saved slots: slot 1 defaults to Edgeville, filled slots show compact place names when a nearby map label is available, empty/edit slots arm a tile save, and filled slots send the existing `WORLD_WALK_REQUEST`, so saved traversal auto-walks rather than teleporting. Saved slots are stored per account in the client cache as `worldmap-waypoints.properties`; there is no server DB state, opcode, packet shape, client-version, cache asset, or schema change. Files: `VoidRift.java`, `SceneryLocsVoidRift.json`, `WorldMapPanel.java`, `mudclient.java`, launch traversal docs. Reversibility is restoring the Enclave-only rift handler/loc list and removing the waypoint row plus local properties helper.

### 2026-07-05 - Newbie guided onboarding retired; skip and veteran hardened

Retired the "I'm new to Classic" guided onboarding option after review found the Tutorial Isle/newbie path too exploitable. Fresh accounts now see only Veteran and Skip cards; stale `void_onboard_track=1` / `void_guided_*` accounts are migrated into the skip/path-choice flow and the old guided tour/fight code plus Tutorial Isle gate/runtime loc loading were removed. Skip/path choice now resumes only after the player is registered, force-saves critical onboarding state, and preflights the full starter kit before committing `void_path`, so full backpacks or add failures cannot consume the one-time kit. Veteran-track players must complete the Archivist first visit before the Herald opens path choice, and the rested/loot-beam demos are gated to the veteran welcome track. Files: `VoidOnboarding`, `VoidWelcome`, `VoidPath`, `VoidVeteranTour`, `VoidHerald`, login/post-login trigger wiring, walk/world-walk handlers, `WorldPopulator`, and the desktop client welcome-card renderer. No packet, opcode, schema, client-version, or cache-asset change; reversibility is restoring the guided menu/dispatch/classes and Tutorial Isle loc loading, then reverting the veteran/kit guards.

### 2026-07-05 - Server agent guide for parallel work

Added `server/AGENTS.md` as the server-subsystem routing guide expected by the root agent instructions. It points future agents at the core server docs, maps major server ownership areas, records server-specific invariants around packets, player cache keys, plugin triggers, tick-thread mutation, runtime artifacts, and gives safe parallel-agent lanes for core systems, plugin/content, data/definitions, QA, and docs work. This is workflow documentation only: no gameplay rule, packet, opcode, schema, cache asset, build script, or runtime behavior changed. Reversibility is deleting `server/AGENTS.md` and this entry.

### 2026-07-05 - Public Android APK refreshed with mobile UI build

Updated the live public Android download at `/downloads/android-apk` to the current July 5 debug APK carrying the shared mobile UI/bank/chat work. The previous public APK (`1762dfbc...`, built June 27) was backed up on the VPS at `/opt/voidscape/backups/android-apk-pre-20260705T155501Z.tgz`, then `/var/www/html/voidscape/Voidscape-Android-Beta.apk` was atomically replaced with the rebuilt artifact (`7f2e9e908c06ae202be6c046f344e37c2fe01925eb1806c80aab754d750aac50`, 17,299,050 bytes). Release signing was not configured on the local build machine, so the public channel remains debug-signed for this refresh; the old and new APKs use the same Android Debug signing certificate, preserving in-place update compatibility for existing sideload testers. Verification: `scripts/build-android.sh --debug`, `apksigner verify --print-certs` on old/new/public APKs, server-side `sha256sum`, public download hash check, and `/api/public` Android row reporting the new SHA-256/size.

### 2026-07-05 - Mobile `/play` Void Glass bank smoke and scroll routing

The TeaVM `/play` phone path now has an authenticated focused bank smoke that opens the real shared bank via mobile keyboard `::quickbank`, seeds/restores a temporary SQLite bank fixture, and proves the Void Glass renderer, search/clear, scroll, withdraw/deposit/deposit-all, loadout save/load, and Close path. `BankInterface` publishes compact bank diagnostics for the web shell, and TeaVM copies those diagnostics into `ui.bank` so the smoke uses live shared-client geometry instead of hardcoded bank coordinates. The web high-level scroll path now routes scroll ticks from touch drags over the open Void Glass bank into the shared bank renderer before falling through to legacy panel scrolling, which fixes `/play` bank drag-scroll while preserving desktop behavior. The legacy custom bank guard was also tightened so `C_CUSTOM_UI` always wins over a stray `want_custom_banks:true`; the legacy `CustomBankInterface` is only for non-custom-UI legacy presets. No packet, opcode, DB schema, cache asset, or gameplay banking rule changed. Verification: `scripts/build.sh`, `scripts/build-web-teavm-spike.sh`, `WEB_TEA_SMOKE_AUTH_DB=server/inc/sqlite/preservation.db scripts/smoke-web-teavm-iphone.sh --no-build --user wbtest --pass voidtest123 --only-bank --out tmp/mobile-qc-web-bank-current`, `scripts/build-android.sh --debug -- --no-configuration-cache`, `scripts/android-smoke.sh --no-build --only-auth-bank --out tmp/mobile-qc-android-bank-current`, and `scripts/android-smoke.sh --no-build --only-auth-chat-tabs --out tmp/mobile-qc-android-chat-tabs-current`.

### 2026-07-05 - Mobile UI QC pass: Void Glass bank touch targets and shared chat strip

Mobile `/play` and Android now carry the current desktop UI direction through the shared client while keeping phone ergonomics. The Android-profile Void Glass bank keeps the same renderer/config contract (`want_custom_ui:true`, `want_custom_banks:false`, `want_bank_presets:true`) but enlarges the mobile-only close/search/action/loadout affordances: the close control is a visible `Close` pill, search clear has a wider hit target, loadout chips/action buttons are taller, and Void Glass menu rows are 28px on mobile while desktop keeps its compact chrome. The Android bank smoke now logs close/loadout/action geometry and proves the Close path instead of force-stopping after banking. The mobile chat strip uses the shared six-band glass tabs (`All`, `Chat`, `Quest`, `Global`, `PM`, `Rpt`) with loose recent-message text and no framed chat container; the web fallback tray also includes Global but remains hidden in normal play. No packet, opcode, DB schema, cache-format, desktop behavior, or launch config change was added. Verification: `scripts/build-web-teavm-spike.sh`, `scripts/build-android.sh`, `scripts/build.sh`, `scripts/smoke-web-teavm-iphone-controls.sh --no-build --out tmp/mobile-qc-web-controls-v2`, `scripts/smoke-web-teavm-iphone.sh --no-build --user wbtest --pass voidtest123 --out tmp/mobile-qc-web-iphone-v2`, `scripts/android-smoke.sh --no-build --only-auth-chat-tabs --out tmp/mobile-qc-android-chat-tabs-v1`, and `scripts/android-smoke.sh --no-build --only-auth-bank --out tmp/mobile-qc-android-bank-v2`.

### 2026-07-05 - Launcher updater hardening review fixes

Follow-up review hardening for the desktop launcher update pipeline (`PC_Launcher/.../VoidscapeUpdater.java`, `scripts/smoke-launcher-update.sh`, `scripts/verify-launch-staging.mjs`). Offline fallback now requires a real `.voidscape-sync-state.properties` record whose managed files still hash-match on disk; a stray/corrupt `Open_RSC_Client.jar` is no longer enough, and update-policy failures such as non-loopback plain HTTP stay fatal instead of being converted to "offline". Play no longer catches every sync failure and launches stale files after integrity/download errors. Launcher async work marks `busy` before the worker thread starts, closing a double-click race. The launcher update smoke now runs from `/` and defaults artifacts to the system temp directory, so repo-local `Client_Base` seeding cannot contaminate the hermetic channel; verified even with `--out tmp/launcher-update-fixpass` (27/27). The staging verifier now requires every manifest file to carry SHA-256 + size metadata and probes every listed payload URL, not just the first file. Added `.gitignore` coverage for `server/logs/gc.log.*` runtime logs. Verification: `scripts/build.sh`, `scripts/smoke-launcher-update.sh`, `scripts/smoke-launcher-update.sh --no-build`, `scripts/smoke-launcher-update.sh --no-build --out tmp/launcher-update-fixpass`, `node --check scripts/verify-launch-staging.mjs`, and adversarial unverified-cache sync probes.

### 2026-07-04 - Desktop launcher auto-update overhaul (startup sync + launcher self-update)

The PC launcher's updater graduates from a Play-time foundation to a full auto-update pipeline (`PC_Launcher/.../VoidscapeUpdater.java`, `Main.java`). It now syncs from the update manifest on every startup as well as on Play (Play falls back to already-verified local files when the update server is unreachable), retries downloads 3x with SHA-256 verify + atomic move, shows aggregate byte progress from the new `file.N.size` keys, prunes files removed from the manifest (state in `<cache>/.voidscape-sync-state.properties`; delete to force full re-verify), and refuses plain-http manifests/downloads for non-loopback hosts unless `voidscape.allowInsecureHttp=true`. The manifest contract is now v2 (`clientVersion`, `file.N.size`, `launcher.sha256/url/size/version` — strictly backward compatible with v1), produced by both the portal's `/api/launcher/manifest.properties` and `scripts/package-friend-beta.sh`, which now ships the launcher jar inside the static channel at `update/launcher/VoidscapeLauncher.jar`. When `launcher.sha256` differs from the running jar, the launcher downloads the new build to `<cache>/launcher/VoidscapeLauncher-<sha8>.jar`, verifies, and re-execs into it with a `--relaunched` loop guard — the originally distributed jar remains a working bootstrap forever, so a launcher restart delivers both new client files and new launcher builds. New CLI: `--sync-only` (headless sync for smokes/CI, prints `SYNC_*` markers, exit 0/1) and internal `--relaunched`; `--no-update` now actually disables sync; ant stamps `Implementation-Version` into the jar (shown as "Launcher build" in Settings). Verification: new hermetic e2e `scripts/smoke-launcher-update.sh` (fresh install / idempotency / repair / prune / self-update staging / loop guard / offline / http refusal) and `scripts/verify-launch-staging.mjs` now validates the hosted update channel (manifest parse, sha shapes, `clientVersion` vs server conf, file + launcher URL probes). Launcher and tooling only: no packet, opcode, cache-format, or server gameplay change. Reversibility: the manifest stays v1-compatible so old launchers keep working; reverting the launcher restores Play-only updating.

### 2026-07-04 - Tutorial Isle: the guided track moves to dedicated gated terrain

Owner playtesting found the guided newbie track confusing on the shared Void Island (all onboarding NPCs visible at once; the veteran-tour Archivist would greet newbies). New custom terrain: a Tutorial Isle (x34-45, y18-40, same sector as Void Island, 2-tile water channels both sides) carved by `patch_tutorial_isle_sector` in `scripts/patch-void-enclave-landscape.py` — three chambers south-to-north (Landing/camp, Ring/spar, Scar/ambush) joined at a lane (x=40), with baked fence rows (LOW_FENCE + VOID_V3_SIGIL flanks) whose lane gaps are script-gated by `void_guided_stage` (`VoidTutorialIsle.java` + walk-handler hooks, council-gate pattern). Guided players rift-teleport there after the lore and rift back beside the Herald when done; Guide/Adept/rats/camp scenery moved off Void Island into `NpcLocsVoidTutorialIsle.json`/`SceneryLocsVoidTutorialIsle.json`. The isle is a SAFE_ZONE in `Point.java` (required by the scoped safe-death contract), inside `inStarterIsland` (leave-block), wilderness-suppressed + named on the client, and baked onto the world map. Old-flow mid-tour saves are re-routed at login. Terrain-only client change: `Client_Base/Cache/video/Custom_Landscape.orsc` + worldmap files must ship (MD5.SUM regen at deploy); no CLIENT_VERSION bump (no new defs — NPCs 864-868 shipped in 10122). Reversibility: drop the isle patch call + registrations; the sector re-bakes to island-only from authentic bytes on the next patcher run.

### 2026-07-04 - Void Island 3-way onboarding (new / veteran / skip)

New characters now pick an onboarding track the moment they land on Void Island, via a server option menu the custom client renders as a glass card picker (prefixes in `VoidOnboarding.OPTION_*`, matched by `mudclient.isVoidscapeWelcomeMenu`). **Skip** stores `void_intro_seen` and opens the path picker immediately (path-choice dialogue extracted to `VoidPath.openPathChoice` so it runs NPC-less; the Herald now delegates to it). **New to Classic** plays the council lore then runs a guided three-station tour (`custom/onboarding/VoidGuidedTour` + `VoidGuidedFight`): a skilling camp (one-time tool kit, new `SceneryLocsVoidIsland.json` with trees + a net fishing spot), a scripted spar against an instanced Void Adept, and a Void Rogue ambush at the north tip — fights use the scoped safe-death contract (instanced, no item loss, respawn by the guide). **Veteran** plays the lore then a Void Archivist tour (`content/VoidVeteranTour`): one-time demos granting a full rested-XP pool (`RestedExperience.grantFull`, added) and an owner-scoped loot-beamed dragonstone that dissolves on pickup, plus a repeatable topic menu (also available at a second Archivist spawn in Lumbridge). Track state lives in `void_onboard_track` (+ `void_guided_*`, `void_vet_*` keys); the welcome menu fires through a new `VoidWelcomeTrigger` from `PlayerAppearanceUpdater` and resumes from `shared/PlayerLogin`. Five new NPCs (864-868, defs on both server and positional client list) bumped `CLIENT_VERSION` to 10122 (all versioned presets + voidbot's default updated). The HUD vitals/FPS stack now also hides while an options menu or card picker is up — it previously overlapped the left card on both pickers. voidbot gained `register`-compatible `design-character` (PLAYER_APPEARANCE_CHANGE, wire id 235) so fresh-account onboarding is scriptable. Legacy mid-flow accounts never see the welcome menu retroactively. Reversibility: restore the inline Herald path block, remove the welcome dispatch/plugins/NPC defs, and revert the version bump; `void_*` cache keys are inert if orphaned.
...
### 2026-06-27 - Void Sparrow wilderness castle promo cinematic

Added admin-only cinematic capture helpers for staged promotional footage around the Dark Warriors Fortress. `::cinematic teamfight [actors] [radius]` now spawns two rune-armored, cape-colored dummy teams that face, swing, move, speak, and show staged damage splats without running real combat, loot, XP, auth, socket, or DB-save paths. `::cinematic sparrowcastle [actors] [radius]` teleports the recording admin to the wilderness castle area, starts the real Void Sparrow scouting view, queues a legal one-tile-at-a-time flyover path across the fortress, and reveals the staged team fight outside the castle. This is local/staff cinematic tooling only: no normal Void Sparrow item behavior, PvP rules, packet shape, client protocol, cache asset, or database schema changed; reversibility is removing the new cinematic subcommands and scene driver branches.
...
### 2026-06-28 - Client camera interpolation toggle

Added a client-only `Camera interpolation` option to the in-game Options list and Advanced Gameplay settings. The default is off, so fresh desktop/web/Android clients snap the camera center and auto-rotation directly to the local player instead of easing behind the character; turning it on restores the prior smooth follow/rotation behavior and saves locally to `clientSettings.conf` as `camera_interpolation`. This is a presentation setting only: no movement/pathing, server tick, opcode, packet payload, gameplay state, cache asset, or database behavior changed.

### 2026-06-28 - Prelaunch Resend email queue

Wired the prelaunch portal to queue account email events and deliver them through Resend when configured. Launch-signup account creation now queues a `signup_confirmation` event after the account, first OpenRSC character, and starter-card marker are committed; delivery failures stay in the durable portal store for retry instead of affecting signup. Token-gated staff endpoints can list email events, retry pending/failed sends, dry-run or backfill signup confirmations, and dry-run or send the launch-live campaign. `/api/health.config.email` reports only safe provider/configuration booleans, staging supports `PORTAL_EMAIL_DRY_RUN=1`, and the launch-staging env template now carries Resend/public-origin settings instead of inert SMTP placeholders. Validation passed with `scripts/test-portal-api.sh`, `node --check web/portal/dev-server.mjs`, shell syntax checks, and `git diff --check`. This changes portal/account communications only: no game server packet, opcode, gameplay rule, cache asset, client runtime behavior, or OpenRSC DB schema changed.
...
### 2026-07-03 - ::queststage honors its documented optional stage (VS-055)

`::queststage [player] [questId] (stage)` documented the stage as optional, and `SuperModerator.setQuest` even carried the `stage = 0` default branch — but the `args.length < 3` gate at the top rejected the 2-arg form with the very usage string that promised it, leaving the default branch dead (S-G §2). Since the `resetquest`/`resetq` aliases route to the same handler, the natural 2-arg reset never worked either. The gate is now `< 2`, bringing the documented reset-to-stage-0 form to life; the 3-arg path is unchanged. Verified live as qabot04 on quest 5: baseline stage 0 → `::queststage qabot04 5 2` → query 2 → `::queststage qabot04 5` → query 0 (`tmp/vs055/02-verify.txt`); build green; `tests/smoke.sh` 26/26. Admin command only; no packet, opcode, game DB schema, player-facing gameplay rule, cache asset, or deployed web payload changed.

### 2026-07-03 - ::rested message wording matches the actual per-second accrual (VS-056)

The `::rested` status line said "Earns one rested minute per offline minute" while the pool actually accrues one rested SECOND per offline second (`RestedExperience.accrue` stores raw offline seconds; `docs/subsystems/progression-balance.md` and the original DIVERGENCE entry both say per-second). The rate is the same 1:1 either way, but the minute wording implied whole-minute quantization that does not exist — sub-minute offline chunks count. The message now says "one rested second per offline second", matching code and docs. Verified live: `::rested` renders the new line (`tmp/vs056/01-new-wording.txt`); build green; `tests/smoke.sh` 26/26 (first post-restart run flaked 23/26, clean on rerun — second sighting of the post-restart warm-up pattern, now recorded in the BUGS.md Watch list). Message string only; no packet, opcode, game DB schema, gameplay rule, cache asset, or deployed web payload changed.

### 2026-07-03 - Tick-loop performance pass 2: idle tick cost cut ~4x, event stage eliminated, GC stalls removed

An 11-dimension audit plus escalating `::loadbots` measurement found the game loop had regressed since the 2026-04-30 pass (idle tick p95 44-84ms at 0 players / 3,658 NPCs; p50 ~100ms at 151 synthetic players with skipped ticks; saturation at 301). Nine verified causes were fixed, none gameplay-visible. Server: `System.gc()` no longer runs on the game thread when a tick is late (it was an unconditional full-GC stall on the very tick that was already late, measured at ~371ms average, and the profiling dump it preceded is now only built when Discord monitoring is enabled); non-player and per-player events run inline on the game thread when the event pool is single-threaded (the default) instead of round-tripping ~3,700 FutureTasks through `invokeAll` per tick; per-NPC `StatRestorationEvent`s park themselves when the NPC has nothing to restore and are re-armed by a centralized hook in every `Skills` mutator, preserving the wall-clock heal anchors exactly; NPC roam rolls are phase-jittered at spawn so the whole world no longer path-builds in lockstep every 5 ticks (cadence, 50% roll, and destinations unchanged); `PathValidation.checkBlocking`/`isMobBlocking` resolve the tile's Region once and use new non-stream first-match `Region.getNpc`/`getPlayer` lookups (insertion order preserved, results bit-identical); `Path.addStep` probes the diagonal only when a diagonal step is actually possible; `RegionManager` region resolution is two lock-free map reads instead of five, `getVisibleRegions` builds no boxed lists (visit order preserved), and `getLocalPlayers` returns an empty set without scanning when nobody is online; the scenery region scan is shared between the scenery and boundary update passes per player per tick; player appearance dedup is a HashMap lookup instead of a linear entry scan (was O(localPlayers²) per observer per tick). Persistence: the SQLite connection now opens with WAL / synchronous=NORMAL / busy_timeout=5000 (bare-file backups must `PRAGMA wal_checkpoint(TRUNCATE)` first; clean close checkpoints automatically), and a `ReentrantLock` on the shared JDBC connection is held across whole `atomically()` transactions — including the game-logger's raw-connection path — closing an interleaving window where a colliding rollback could destroy an in-flight player save. Runtime: `scripts/run-server.sh` launches the existing `runserverzgc` target (ZGC, fixed 2g heap) instead of the legacy Java-8 G1 flag line whose dead `-XX:+UseBiasedLocking` and 64m young-gen pin caused ~17ms young pauses every ~12s; both ant targets now write rotating `logs/gc.log`; the G1 fallback drops the dead flags and adds `-XX:+ExplicitGCInvokesConcurrent`. Client (deliberate QoL divergence from the authentic one-packet-per-frame drain, replacing the in-code TODO): `mudclient.checkConnection` drains up to 32 inbound packets per 20ms frame with a mode/stream guard, so the server's batched per-tick flush renders in one frame instead of smearing over several. Separately diagnosed, no code change: the "idle tick creep" over uptime is macOS efficiency-core scheduling of the mostly-idle game thread, not a leak (heap flat over 25 min; the step reverses without restart) — Linux staging numbers are the only valid regression baseline. Measured on the same machine, same world (idle, 0 players, 3,658 NPCs): tick p50 24-39ms -> ~8ms, tick p95 44-84ms -> ~14ms, npc stage p95 47-62ms -> ~11ms, events stage p95 13-15ms -> ~0.1ms. Load-curve results live in docs/PERFORMANCE.md. Wire format, PID order, combat timing, NPC movement cadence, and visibility semantics unchanged; build green; `tests/smoke.sh` green (see PERFORMANCE.md for the post-pass verification list).

### 2026-07-03 - drawBoxAlpha green-channel mask decompiler artifact fixed (UI Slice 1, sanctioned global pixel change)

`GraphicsController.drawBoxAlpha` blended the green channel through the mask `(color & 'ￄ') >> 8` — the char literal is 0xFFC4, a decompiler artifact that let two bits of the *blue* channel bleed into every translucent box's green blend since the vendor snapshot. The mask is now `0xFF00`. This is one of the two sanctioned global pixel changes in `docs/UI-STYLE-GUIDE.md` §8: it affects every translucent rectangle in BOTH skins (authentic included), shifting blends by at most ~3/255 per channel — visually imperceptible but no longer mathematically wrong. Verified: build green; full `/scenario/ui-panels` sweep + bank + login captures on the new build show no visible artifacts (before gallery: `tmp/workbench/baseline-ui/`, after: `tmp/workbench/screenshots/20260703-1600*`). No packet, opcode, DB schema, gameplay rule, or cache asset change.

### 2026-07-03 - drawShadowText unified onto the auto-shadow convention (UI Slice 1, sanctioned global pixel change)

`GraphicsController.drawShadowText` drew its shadow at offsets (-1,0)/(0,-1) in 0x0F0F0F while the automatic in-game shadow pass (drawColoredString) draws at (+1,0)/(0,+1) in black — two shadow conventions, and because 0x0F0F0F != 0 the explicit shadow passes were themselves re-shadowed by the automatic pass when logged in (double draw). drawShadowText now uses the auto-pass offsets with pure black, which also suppresses the recursive shadow. Second sanctioned global change per `docs/UI-STYLE-GUIDE.md` §8; affects the handful of drawShadowText call sites in both skins by one pixel of shadow direction. Verified in the same capture sweep as above. No packet, opcode, DB schema, gameplay rule, or cache asset change.

### 2026-07-03 - UI standardization program: style guide + Slice 1 token foundation (UiSkin/UiAnchor)

Kicked off the UI standardization program ("One Void — Authentic-Plus", unanimous 3-lens design-panel winner; full spec and 10-slice roadmap in the new `docs/UI-STYLE-GUIDE.md`). Slice 1 lands the foundation: new Client_Base-pure `orsc.graphics.gui.UiSkin` (complete void-theme palette as `voidscape.ui.*`-overridable tokens, alpha ladder, named font constants, WORLD_* exempt colors, SpriteHook nine-slice indirection with structural code-drawn fallback, and shared chrome helpers panel/titleBar/button/closeButton/textField/listRowFill/tooltip/plaque/pushClip-popClip) and `orsc.graphics.gui.UiAnchor` (centering/safe-area math, promoted from the void-arena dialog helpers which now delegate to it). Every voidscape-skin hex literal in the audited surfaces now reads a token: mudclient glass kits + ornate panel chrome, the five vBox tab blocks, equipment/settings/player-info sub-tab strips, title/path/void-arena/login/farm-sim kits (the steel-grey login cards are now void-dark with gold framing — the one deliberately visible restyle, since `useVoidscapeLogin()` is constant-true these cards show in all presets), advanced-settings modal (+ standard UiSkin.closeButton), skill tooltip (UiSkin.tooltip), scout timer, FPS chip (green family retired to void tokens), XP badge (voidscape branch only), Void Colossus boss HUD, plus Menu.java VOIDSCAPE_UI_* aliases, BankInterface VG hover/ring aliases, CustomBankInterface exact-match UI_* aliases, WorldMapPanel window/title colors, OnlineListInterface named grey, and the PC bootstrap accent in ScaledWindow. GraphicsController additionally gained `drawBorder(x,y,w,h)` (sane-argument-order delegate to drawBoxBorder), public clip getters, a drawCircle fix to respect the clip rect (previously ignored it — minimap dots can no longer bleed outside a clip), and four semantic `@col@` codes (`@hdr@`/`@acc@`/`@dis@`/`@bod@`) that map server-driven text onto theme tokens. Zero layout/geometry changes anywhere; classic-skin code paths untouched except the two sanctioned fixes above and the drawCircle clip correctness. Verified: `scripts/build.sh` green; workbench `/scenario/ui-panels` (14 panels) + bank + login-home + existing-user captures on the new build — settings modal, login cards, and HUD chips visibly converge on the palette while already-canonical surfaces (skills panel, glass kit) are pixel-stable; style-guide grep gate (`0xf3d46b|0xf1d56c|0xb8914f|0xd0a030|0x9a7a31|0xD8C27A` in mudclient.java) returns zero hits. Remaining slices (workbench accessor hardening, Panel.java dual-skin, bank overhang repair, interface fleet, auction house, modal kit, trade/duel/shop skin, HUD unification, responsive hardening) are specified in the style guide §9. Client rendering only; no packet, opcode, DB schema, or gameplay change.

### 2026-07-04 - UI Slice 2: workbench QA channel reads layout accessors; standing regression matrix

The workbench's synthetic clicks stop guessing coordinates: `clickSavedUserLogin` reads new `workbenchLoginHome*/workbenchExistingUserOk*` accessors on mudclient, and the auction-house tab/category/row clicks read new `workbench*` accessors on AuctionHouse — in every case the accessor and the draw call now derive from the same private constant/helper, so paint and click can no longer diverge (two real off-by-one login drifts were absorbed in the process). The long-broken `/scenario/auction-house-open` ("tab 1 did not activate") was root-caused as the scenario closing its own window: `clearWorkbenchBlockingUi()` ran after opening the AH and `setVisible(false)`d it; a new `clearWorkbenchOverlayDialogs()` dismisses blocking dialogs without touching the AH at that one call site. New `scripts/ui-regression-matrix.sh` captures `/dev/viewport` 0-5 × `/scenario/ui-panels` into diffable stable paths and byte-compares against a `BASELINE=` dir (PASS/DIFF/MISSING, exit 1 on regressions). Verified: scenario now returns ok with all 5 captures; matrix run green across all 6 presets (baseline `tmp/workbench/matrix/20260704-080716/custom`). QA tooling + behavior-preserving refactor only; zero visual or gameplay change.

### 2026-07-04 - UI Slice 3: Panel.java dual-skin — legacy widget fleet joins the void theme

Panel.java (the authentic RSC widget toolkit) gains a flag-gated void skin using Menu.java's exact gate (`C_CUSTOM_UI || F_ANDROID_BUILD && S_WANT_CUSTOM_UI`); with the flag off every path is byte-identical to classic, colorA-L untouched. Skinned: button-background plates and decorated boxes draw the UiSkin panel recipe (dark body, gold bevel, inner border) instead of blue-steel gradients; scrollbars get VOID_BOX tracks with PURPLE_EDGE thumbs; toggle-button checkboxes get FIELD_BG + gold check; the five duplicated list state-color blocks collapse into one `resolveListColor` (classic values preserved exactly, including the alt-flag quirk on clicked rows); touch-height list rows get a PURPLE_SELECT hover fill; base panel text is parchment on the void skin. The orphaned TOGGLE_BUTTON renderer finally gets its `addToggleButton()` adder. The void quest-list builder now uses the Slice-1 semantic codes — gold header, parchment for not-started (red wall retired), green/yellow state colors kept; the classic builder is untouched. Verified: build green; workbench captures of quests/friends tabs show the conformed lists and scrollbar; classic path unreachable-changed only (flag-gated). Client rendering only.

### 2026-07-04 - UI Slice 4: bank usable at base resolution (both bank paths)

Two bank renderers exist: the live Void Glass path (`BankInterface.renderVoidGlassBank`, active because `want_custom_banks=false`) and the dormant `CustomBankInterface` custom path (active on presets that enable it). Both repaired. Void Glass: opaque `VOID_BODY`@232 backing behind the bank grid and inventory tray (glass margins keep their translucency; the 3D world no longer bleeds between slot rows), and the panel publishes its real rendered bottom (`getVoidGlassPanelBottomY`) so both chat renderers (loose voidscape + classic fallback) clamp message lines below the panel while banking — system chat no longer prints across the L1/L2/L3 preset row and action strip. CustomBank path: `updateLayout`'s `Math.max` floor defeated the small-screen clamp (580px panel on a 512px frame, panelX=-34); a `narrowDesktopLayout` tier (desktop, ≤560px, non-web-preset) clamps the panel to the viewport and adopts the proven web-tier vertical scheme (3 rows, 32px cells, compact offsets) so the panel fits 334px-tall frames; ≥640px frames byte-identical. Note: the pre-fix `/state` bank layout numbers came from the dormant path — live-path observability is a Slice 2-pattern follow-up. Verified: build green; workbench captures at 512x334 show the bank fully on-screen with opaque grids and a legible toolbar; auction/panels matrix green. Client rendering only; no packet, opcode, DB schema, or gameplay change.

### 2026-07-04 - Glass pivot: translucent light-purple glass becomes the standard panel treatment

Owner direction after seeing the Slice-4 opaque bank backdrops: the see-through "Void Glass" look is the brand — panels must keep their translucency, and the opaque backings were rejected. The VG palette is promoted into UiSkin as first-class tokens (`GLASS_BODY` 0x160B2C, `GLASS_RIM` 0x8A6BD8, `GLASS_SHEEN`, alpha tiers `A_GLASS` 112 for hero/grid surfaces and `A_GLASS_TEXT` 168 for text-dense cards) with a canonical `UiSkin.glassPanel()` helper (body + upper sheen + black outer border + lavender rim). Reverted: both opaque grid backdrops (VG bank and the dormant CustomBank path). Converted to glass cards: Panel.java's void-skin button-background plates and decorated boxes, the advanced-settings modal, the login/recovery card frame, the farm-sim modal, and the title-menu frame. Kept from Slice 4 regardless of the pivot: the chat clamp below the bank panel and the CustomBank narrow-frame layout tier (geometry, not opacity). `docs/UI-STYLE-GUIDE.md` §1.1/§1.3 amended with the glass mandate ("opaque backdrops are banned on standardized surfaces"). Verified by workbench captures of bank/settings/login/quests on the rebuilt client. Client rendering only.

### 2026-07-04 - UI Slice 5: interface fleet migrated onto the InterfaceChrome glass kit

New `com.openrsc.interfaces.misc.InterfaceChrome` (glass window = glassPanel + header strip + gold title + standard X at shared closeX/closeY, sectionStrip, hover-returning button) consuming UiSkin. Migrated off the grey family (0x989898 plates, 0x333333 button copies, 0x6580B7 blue strips): SkillGuide, DoSkill, LostOnDeath, QuestGuide, TerritorySignup, PointsToGp, ExperienceConfig, AchievementGUI, IronMan (brown family), PointInterface, OnlineList, PartyGUI, ProgressBar. Every close button's draw and hit-test now derive from the same chrome coordinates. NComponent overlays got token recolors only (freeze rule honored; OnlineList keeps no rim — adding one would need a new primitive). Named bugs fixed en route: AchievementGUI off-panel footer re-anchored; PartyGUI/ProgressBar now re-anchor on window resize (resize-gated so their drag-to-move gesture survives); DoSkill close hit-test no longer computed against a stale mid-frame width; LostOnDeath's transposed drawLineVert args (color=2, an invisible divider) fixed by tokenization; PartyGUI's malformed 7-digit hex retired. The in-world hover hint (item/actor at 6,14) gets a glass plaque behind it on the void skin, and UiSkin.tooltip itself moved to glass (GLASS_BODY/A_GLASS_TEXT + GLASS_RIM) — the skill tooltip inherits it. Skill-guide window unverifiable live on this preset (want_skill_menus false) — compile-green, pattern-identical to verified siblings. Client rendering only.

### 2026-07-04 - UI Slice 6: auction house joins the glass theme

AuctionHouse.java's fourth-era chrome (navy/olive plates, tan category tiles, light-grey tab strip, green header rows, white search box) migrated to the glass system: InterfaceChrome.window card with gold title and standard X (wired to the existing auctionClose() packet path), tabs as UiSkin.button states (PURPLE_SELECT + gold underline when active), category tiles VOID_BOX/GOLD_LINE with GOLD_HOT hover, sectionStrip header rows (green retired to gold), UiSkin.textField search. The top tab row no longer overlaps the location banner/FPS chip: panel origin clears the overlay top safe area via the shared layout constants — which the workbench accessors also read, so the QA click helpers tracked the move automatically (/scenario/auction-house-open re-verified green, all 5 captures). A pre-existing hit/draw overlap band that let one click select-and-instantly-cancel an auction was tightened during the migration. At the 512x334 clamp the inventory picker keeps all 6 rows clickable at the cost of ~2-5px clip on the last row (lesser evil vs unlistable slots). Client rendering only; packet writes untouched.

### 2026-07-04 - UI Slice 7: modal kit for the black-box dialog family + classic dialog centering

The eight legacy "black box" modals (welcome, server message, wilderness warning, logout, change password, the three social prompts, report abuse name+grid, InputX) now draw through UiSkin.modal (glass card + gold title strip; widths 400/468 clamped to frame, centered via UiAnchor) on the void skin, with the classic black-box branch preserved byte-identical behind the skin gate. Every migrated dialog had its paint coordinates and click rects re-derived from ONE shared set of variables, which surfaced and fixed three real input bugs that affected both skins: the social prompts' dismiss check dropped a y-term (any click closed the popup, making Cancel dead code — its rect was also 50px right of the drawn text), the change-password dismiss rect never matched the drawn box (Android keyboard offset could dismiss on an inside click), and InputX drew text/OK/Cancel against a stale 200 half-width on widened prompts. System-update and loading banners lost their literal-256 centers. Second half: the classic trade, duel, and both confirm dialogs re-anchor via shared tradeDuelDialogX/Y() helpers (UiAnchor) — the old (22,36) literal is exactly the centered position of their 468x262 box at 512x334, so classic frames are pixel-identical and wider frames now center; every hit band rides the same variables; trade/duel/confirm packets (8/55/77/104/176/197/230) byte-untouched; shop already width-derived, left as-is. Verified: build green; workbench captures of the glass welcome, report-abuse name entry, and add-friend prompt; click-path checks — report Cancel closes, social Cancel closes (previously unreachable), inputXAction returns to ACT_0. Client rendering + client-side hit-rect corrections only; no packet or gameplay change.

### 2026-07-04 - UI Slice 9: one modality table (ESC/Android-back close the top modal) + HUD unification

New `closeTopVoidscapeModal()` in mudclient: a single 34-entry priority table (seeded from getWebOverlayDialogName's order) that closes the topmost open modal via that modal's own existing close path — including the ones that must send packets (trade/duel declines 230/197, shop close 166, bank close 212, bank-PIN cancel, auction close, options-menu dismiss 116) — and refuses correctly where the modal itself refuses (welcome during recovery countdown, points-to-GP during skill select). Wired into the ESC key path (replacing three subsumed standalone branches; key consumed only when something closed) and the Android back-button path — bank, bestiary, farm-sim, and void-scout become back-closable for the first time. ESC-close now also works in the classic skin (behavior only; zero classic pixels changed). Mouse-wheel capture routing deferred. HUD sweep (void skin only): desktop chat tabs adopt the mobile icon+tint treatment (PURPLE_SELECT active, token labels; click rects untouched); the duplicate bottom-right wilderness skull is deleted (plaque copy kept); the kill feed anchors to getGameWidth()-5 instead of literal 507; the legacy XP counter keeps its distinct data but wears badge-style token chrome; UiSkin.timerBanner() promoted from the scout-timer recipe now also renders the system-update countdown and EXP-elixir timers as stacked top-center plaques (classic keeps the legacy texts byte-identical). Verified: build green; ESC live-closes the bank (staged VG path) and auction house via workbench /input/key; HUD captures clean. Three latent bugs found and filed in docs/BUGS.md Intake (duel-confirm decline sends the trade opcode; auction stale state on server-driven close; worldMapPanel null-deref risk on Android back). Client-side only.

### 2026-07-04 - UI Slice 10: responsive hardening + pre-game polish

First-run window sizing: with no persisted clientSettings.conf window settings, the client now auto-picks the largest viewport preset that fits the usable screen (inset-aware, same math as the manual picker) instead of defaulting tiny on large displays; any persisted choice always wins and the auto-pick itself is never persisted. Classic chat-tab hit-tests and sprite draws now read one shared helper set (algebraically identical at every width). The chat drag/wheel gesture zones unify on one 66px game-coordinate constant — the old wheel test was effectively a 63px band (75px component-space minus the 12px footer), so wheel capture grows 3px and finally matches drag. Pre-game login cards center vertically on tall windows via UiAnchor.centeredCardY with their historic Y as the floor (byte-identical at 334; per-screen offset arithmetic in the slice notes); the login background is cover-fit (aspect preserved, centered, cropped) instead of stretched — byte-identical when aspects match. The AWT "Connection lost" box centers from real applet dimensions instead of literal 512x344 (and was found to be unreachable on the modern pipeline — software-renderer move filed as follow-up). New desktop-only "Display" category in Advanced Settings (rail entry appended) exposing Screen size / UI scale / Scale filter through the existing ScaledWindow plumbing — no new persistence or packets. Latent findings filed: chat-tab sprite name inversion, new-user email card 2px overflow (pre-existing), settings Interface category overflow at small heights, reconnect-box invisibility. Verified: build green; login screen cover-fit and card position confirmed at 512x334; Display rows render live values (512x346/Window/Crisp). First-run auto-pick not exercised live (would require clearing this machine's saved settings); logic mirrors the manual picker's fit test. Client-side only.

### 2026-07-04 - UI Slice 8: trade/duel/shop wear the glass skin (zero-geometry reskin) — program complete

The last classic-chrome holdouts — trade, duel, both confirm screens, and the shop — get the glass treatment on the void skin: glassPanel backdrops over the exact rects the classic fills covered, VOID_HEADER title bands, VOID_BOX cells with PURPLE_SELECT selection, tokenized text roles (GOLD_TITLE titles, GOLD_HEADER sections, TEXT_BODY items, GOOD/BAD semantics, GOLD_HOT status lines), and shop prices highlighted via the slice-1 @acc@ code spliced into the shared strings (zero-width, positions unchanged). Risk containment for the quarantined slice: colors only at byte-identical geometry — a diff grep for packet/click identifiers (newPacket/putShort/mouseButtonClick/etc.) returns zero hits, every hit band and loop bound is untouched context, and the classic branch survives verbatim (char-escape literals confirmed byte-identical). Two deliberate judgment calls: shop stock counts stay green (GOOD) because the on-screen legend says "Shops stock in green", and Accept/Decline sprites stay sprites. Verified live: shop opened at the Lumbridge general store renders as a glass card, item selection shows the purple cell + gold "buy for 1gp each" line, and ESC closes the shop through the modality table's packet-166 path. Trade/duel flows not exercised live (needs a second account); geometry-invariance plus the packet-diff grep bound the risk, and a two-account voidbot QA pass is the recommended follow-up. This closes the 10-slice standardization roadmap in docs/UI-STYLE-GUIDE.md §9. Client rendering only.

### 2026-07-04 - UI Slice 8 addendum: trade/duel flows verified live with two accounts

Follow-up to the slice 8 entry: created a second QA account (uiqa2, regular player, cloned from wbtest's row + essential side tables in the dev DB) and ran two workbench clients (ports 18787/18788) through the full flows. Verified end-to-end on the glass UI: mutual trade request, glass trade window, item offer, both-sides accept (packet 55), glass confirm screen with red no-reverse warning, both-sides confirm (packet 104), "Trade completed successfully" with 1 coin actually transferred; mutual duel request, glass duel window with tokenized Duel Options checkboxes, and ESC on one side sending the duel decline (packet 197) via the modality table with the counterparty receiving "Other player left duel screen". The quarantined slice is now fully verified. QA-only DB addition; no code change.

### 2026-07-04 - Client-side clan/party code removed (clans and parties will never ship)

Owner decision: clans and parties are permanently out of scope, so the dead client code is removed rather than parameterized (supersedes the "Clan/Party parameterization" backlog item in docs/UI-STYLE-GUIDE.md §7). Deleted the five classes (Client_Base `interfaces/misc/clan/{Clan,ClanInterface}`, `interfaces/misc/party/{Party,PartyInterface}`, `interfaces/misc/PartyGUI`, 2,752 lines) and excised every reference from mudclient, PacketHandler, ORSCharacter, ORSCApplet, InputImpl (Android), OnlineListInterface, and the enums (MessageTab.CLAN, InputXAction 14-21, MenuItemAction clan/party entries). Inbound opcodes 112/116 were deleted outright — safe because packets are length-framed and drained per read (Network_Base), so unhandled opcodes fall through to handleUnknownPacket without desync. Wire-format parsing is preserved everywhere a clan/party field is embedded in a live packet: SEND_SERVER_CONFIGS bytes 7/64 (S_WANT_CLANS/S_WANT_PARTIES) are read but no longer applied, GAME_SETTINGS bytes 11/36 are consumed-and-ignored, and the player-appearance clan-tag flag byte + string is consumed and discarded. MessageType.CLAN_CHAT (wire type 9) stays declared; such messages now land only on the All chat tab. Config keeps S_WANT_CLANS/S_WANT_PARTIES declared (reflection-based config-file compat, never read) and C_NAME_CLAN_TAG_OVERLAY keeps its historic name but now only controls the overhead name-label overlay. Chat tab 5 is always Report Abuse; the social side panel is always the two-tab Friends/Ignore layout. Server-side clan plugin removal is separate work; opcode ordinals and CLIENT_VERSION untouched.

### 2026-07-04 - HUD vitals panel: Hits + Prayer glass bars under the FPS chip

New always-on vitals readout for the void skin (all platforms), replacing the old text-only mobile-web-only vitals overlay (which still wore pre-token green-family colors): Hits and Prayer render as stacked translucent glass bars (GLASS_BODY body, GLASS_RIM border, proportional fill at alpha 96 — GOOD green for hits flipping to BAD under 30%, PURPLE_BRIGHT for prayer, TEXT_BODY "Hits 90/90" labels) anchored under the desktop FPS chip on the left, right-aligned under the mobile shell chip. Inspired by RSCRevolution's Hits/Prayer/Fatigue panel, minus fatigue (not a voidscape mechanic), restyled to the glass mandate. Hidden while banking (rides the FPS overlay's existing bank gate) and off entirely on the classic skin. Verified: build green; workbench capture shows both bars with correct values and fills at 512x334. Client rendering only.
...
### 2026-07-04 - Bottom chat strip redesigned as a glass tab bar + Global chat history tab

The desktop void skin's bottom chat strip is now code-drawn glass instead of the classic CHATTABS sprite: six even bands across the full frame width (All | Chat | Quest | Global | Private | Report Abuse) with VOID_BOX idle / PURPLE_SELECT active fills, VOID_LINE dividers, a GLASS_RIM top hairline, GOLD_HOT selected labels, DANGER_GLYPH for Report, and the classic 200-tick FLASH cadence per tab (new activity counter for Global). Draw and click both consume one voidscapeChatTabBand() helper family (integer-division bands, remainder-safe), with the same bottom-row click gate as classic so world clicks can't leak. New MessageTab.GLOBAL (appended; lookup() verified caller-free, nothing wire-relevant) backed by a fourth history list with identical dimensions, wired through the show/hide dance, all reposition branches, wheel/scroll routing, and the click-to-PM path; MessageType.GLOBAL_CHAT (::g chat, global quest broadcasts, Discord relay, announcements — the only global-family wire type) routes into it additively, keeping its existing All-tab/overlay behavior. Classic skin: provably dead-branch-only — 5-tab sprite strip byte-identical; mobile/web void dock unchanged at 5 tabs (Global is desktop-void only for now). Verified live: build green; six-tab strip renders; ::g message flowed into the Global tab's own history; Report tab still opens the (glass) report flow. Client rendering + client-side routing only; no packet change.

### 2026-07-05 - Void Glass bank survives legacy custom-bank config drift

Ryan's live launcher session pulled the new `10122` client but still showed the old bank because production was advertising `want_custom_banks: true`; that woke `CustomBankInterface.onRender()` and bypassed the superclass `BankInterface.renderVoidGlassBank()` path even though `C_CUSTOM_UI` was true. `CustomBankInterface` now treats the legacy custom-bank renderer as active only when `S_WANT_CUSTOM_BANKS` is true and `C_CUSTOM_UI` is false on the active client, and its deposit/withdraw/hotkey paths use the same effective mode. The live server config was reset to the intended `want_custom_banks: false`, the update channel was republished with client SHA-256 `978d487522f6e206f1a50d209ae744377f0dabf118ca1b92560f4584c2e4c9a7`, and cache/update permissions were normalized so the portal manifest can read every packaged file. Evidence: `scripts/build.sh` green; deployed package `tmp/live-client-ui-configfix-20260705T044847Z`; static and portal launcher syncs both fetched the fixed jar; live backup is `/opt/voidscape/backups/ui-configfix-20260705T044939Z`. Client render/input gate plus production config only; no packet, opcode, DB schema, cache asset, or gameplay rule changed.

### 2026-07-05 - Mobile `/play` and Android inherit Void Glass bank parity

Audited the desktop UI pass against mobile surfaces and kept the bank fix in the shared client instead of adding a second mobile renderer. `BankInterface.renderVoidGlassBank()` now follows `C_CUSTOM_UI` on Android-profile clients as well as desktop, so phone `/play` and the native Android APK use the current Void Glass bank under the intended launch config: `want_custom_ui:true`, `want_custom_banks:false`, `want_bank_presets:true`. The old `CustomBankInterface` path remains available only for deliberate non-custom-UI legacy custom-bank presets, so a stray `want_custom_banks:true` no longer masks Void Glass on Android-profile clients either. Phone/Android search gained a mobile-only clear target while desktop search visuals stay unchanged, and Android-profile Void Glass grids now support touch-drag scrolling without changing desktop click behavior. TeaVM diagnostics now expose `ui.bankOpen` and `ui.bankRenderer`, and Android bank smoke markers include `renderer=voidGlass`, scrollbar geometry, login geometry, and direct loadout-save behavior for the Void Glass menu. The Android smoke harness now verifies bcrypt-backed auth fixtures, types credentials slowly with shared-client text-length proof, seeds a full 192-slot bank fixture so tall Void Glass layouts must scroll, and accepts renderer metadata in bank action logs. The Android stub `ScaledWindow` now exposes the shared desktop viewport helper signatures so Android keeps compiling when shared settings code references them from unreachable desktop-only branches. The `/play` shell also keeps diagnostics buttons visible over the desktop launcher handoff when `diag=1`, preserving the controls smoke/debug path without changing normal desktop handoff behavior. No packet shape, opcode ordinal, DB schema, cache asset, or gameplay rule changed; mobile bank actions still use the existing bank and preset packets. Verification: `git diff --check`, `scripts/build.sh`, `scripts/build-web-teavm-spike.sh`, `scripts/build-android.sh`, `scripts/smoke-web-teavm-iphone-controls.sh --no-build --out tmp/mobile-ui-parity-web-controls`, and `scripts/smoke-web-teavm-iphone.sh --no-build --user wbtest --pass voidtest123 --out tmp/mobile-ui-parity-web-iphone-wbtest` green after installing `playwright-core` outside the repo at `/tmp/voidscape-playwright-smoke`; an earlier full iPhone smoke with the default `test/test` credentials failed at login only (`expected logged-in title, got Voidscape`) and passed with the authenticated fixture. Focused Android bank smoke passed with `ANDROID_SMOKE_AUTH_USER=wbtest ANDROID_SMOKE_AUTH_PASS=voidtest123 ANDROID_SMOKE_AUTH_DB=server/inc/sqlite/preservation.db scripts/android-smoke.sh --no-build --only-auth-bank --out tmp/mobile-ui-parity-android-bank-rerun`, proving `ANDROID_SMOKE_BANK_OPEN renderer=voidGlass`, search/clear, mobile grid-drag scroll, withdraw, loadout save/load, deposit, deposit-all, preset persistence, and fixture restore against `10.0.2.2:43596`.

### 2026-07-05 - Voidscape stats panel gets full-height skill detail treatment

The Voidscape-skinned player-info skills sub-tab now uses a wider right panel on normal desktop presets, fills the available side-column height, and replaces the old compact summary/equipment/hover-tooltip stack with a slimmer stats summary, 17px font-1 balanced skill rows, base/cap level values such as `90/99`, fixed per-column value rails with a consistent XP-lock gutter so uneven levels like `90/99`, `3/99`, and `40/99` line up vertically all the way down, per-skill XP progress bars, hover-only unlocked XP-lock icons, and a pinned bottom detail footer. The footer shows exact total XP while idle and exact skill XP, next threshold, XP-to-go, and progress while hovering; the obsolete Equipment Status section was removed from the Voidscape path only. Compact web keeps its tiny abbreviated layout and skips the new footer/progress treatment. Client rendering only; no packet, opcode, server, DB, cache, or gameplay change.

### 2026-07-05 - F2 vitals HUD toggle

F2 now toggles the Voidscape vitals stack (the side FPS/Hits/Prayer HUD) on the desktop client instead of the side-menu overlay shortcut; the side-menu overlay remains available through `::overlay` and the Advanced Settings Interface row. The vitals toggle is client-local and only hides the void-skinned vitals/FPS stack, with no persistence or packet changes. The existing `::coords` command path was live-verified against the current player tile and already calls `Player.tellCoordinates()`, so no new server command handler was needed. No opcode, schema, cache asset, or gameplay rule changed.

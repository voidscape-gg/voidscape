...
### 2026-07-02 - Bank preset persistence on SQLite (quote-corrupted rows + closed ResultSet)

Bank loadouts/presets never survived a relogin on SQLite deployments; two stacked server bugs meant every login loaded zero presets and the follow-up logout save then wiped the stored ones. First, `JDBCDatabase.executeQuery` returned the `ResultSet` out of a try-with-resources that closed its `PreparedStatement`, and a closed sqlite-jdbc `ResultSet` silently reads as empty — the consumer now runs inside the statement's scope, which fixes every `executeQuery` caller. Second, the sqlite `bank.addBankPreset` query wrapped its hex-blob parameters in double quotes on top of the single quotes `NamedParameterQuery` already adds, so SQLite's double-quoted-string fallback stored the single quotes as part of the data and hex-decoding on load would have failed anyway; the template quoting was removed. Also extended the 2026-07-02 custom_ui gate relaxation to `BANK_CLEAR_PRESET` in `InterfaceOptionHandler`, which had the same want_custom_banks-only gate as load/save. Existing dev-DB preset rows written in the corrupted format were purged (all were empty presets). Verified live: save loadout → relogin → loadout intact and loadable. No packet shape, opcode ordinal, DB schema, cache asset, or gameplay rule changed.

...
- [ ] Normal public startup refuses a missing, unreadable, malformed, partial/wrong-shape, symlinked, or non-private portal store without changing its bytes. On a genuinely fresh host, root prepares `/var/lib/voidscape-portal` as `voidscape:voidscape 0700`, then the absent-only `--initialize-store` command runs as `voidscape`; the file is `0600`, the audit line is archived, and the initializer is not part of `ExecStart`.
- [ ] Portal and game writers are quiesced; `/var/lib/voidscape-portal/dev-store.json` and a matching SQLite `.backup` (or consistent MariaDB dump) are hashed, stored off-host, and restored together in an isolated rehearsal. Do not make a bare copy of a WAL-mode SQLite main file. A founder CSV export is not accepted as a portal-state backup.
- [ ] Public `/api/health.storage.storeReady` and `/api/health.config.publicReady` are `true`, `abuseHashSaltConfigured` is `true`, and `issues` is empty; runtime store damage returns `503` without exposing filesystem paths or detailed failure reasons.

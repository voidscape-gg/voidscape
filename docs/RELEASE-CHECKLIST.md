...
- [ ] Android day-one distribution: the verified Play Store listing URL points at the approved production track and the checked release-signed direct APK URL and SHA-256 are independently public and tested. Advertise both as first-class choices.
...
- [ ] `production_command_lockdown: true` for public launch. Promote selected player `542` / `name123` offline with the guarded helper and archive its exact account-link/rank/backup hashes, but keep authentication open until the post-cutover private boot; there, require a real `name123` login plus harmless `::loadbots status` owner-only proof. Verify the separate non-owner staff denial as well.
...
- [x] iPhone launch scope decision is recorded: Safari at `https://voidscape.gg/play/` is a supported July 18 browser option. This is not a native iOS/App Store claim. Physical iPhone Safari/Home Screen QA and the final iPhone release audit remain **OWNER-WAIVED / NOT PASSED**; this checkbox records the accepted launch scope, not a device-QA pass.
...
- [ ] Slice 8E Android promotion gate is complete only after Google approves the submitted 100% Production rollout and ordinary players can receive `9 / 1.0.8`, and after the checked direct APK is independently reachable. Internal Testing is already available, Play quick checks passed, physical Android is explicitly owner-waived/not passed, browser-based iPhone Safari support is approved with physical Safari/Home Screen QA owner-waived/not passed, and hosted TeaVM HTTPS/WSS verification is complete at `tmp/launch-staging-hosted-c519a-final-signup-chrome-pass/web-deployment/summary.json`.
...
- [ ] Final prelaunch readiness report is archived with both Android physical QA and iPhone physical Safari/Home Screen QA explicitly owner-waived/not passed. The iPhone browser option is launch-supported despite that accepted risk; do not supply synthetic device reports to turn either row green:
...
- [x] Historical Slice 8D launch staging bundle was generated locally with the production target URLs:
...
- [x] At that historical Slice 8D checkpoint, the local bundle's server config/database/client-cache checksum tables, direct artifact hashes, forbidden-file/secret scan, and all 538 TeaVM build-manifest files passed under `tmp/launch-staging-8d-final*`; no remote sync, service restart, signup, Play upload, or production mutation had yet occurred.
- [x] The final clean-commit production bundle was generated with `--android-release` at commit `6bcdf8607c40848479b9456c1528d88465c26a10`; `MANIFEST.txt` reports `promotable=true`, protocol `10132`, checker-passed release Android, production URLs, and no blockers.
- [ ] Generated `RELEASE-HANDOFF.md` from the final bundle is filled with actual backup paths, rollback owner, and the recorded source-publication decision, then archived with `MANIFEST.txt` and verifier output. Source remains `publication_pending`; this does not settle any separate AGPL obligation.
- [x] Android public-channel decision is recorded: Google Play and the release-signed direct APK are both first-class launch paths. The owner accepts the physical-device waiver/not-passed state for this launch; the signed artifacts and emulator proof do not erase that risk.
...
- [x] `/api/public` shows Android `10132` with SHA-256 `33f2c22a6bfc72782c3c39d73d99dea0f44789eb230d1bc5c5b32f79e27c7324`, and `/downloads/android-apk` streams those exact checked bytes from the private production artifact path.
...
- [x] Public `/api/health.config.publicReady` is `true`, `abuseHashSaltConfigured` is `true`, and `issues` is empty; endpoint output exposes only booleans, never secret values.
...
- [ ] `PORTAL_LAUNCH_FREE_CARD_HOURS=0`; keep signup live during preparation, then use `scripts/manage-portal-roster-freeze.sh freeze` for only the approximately ten-minute final backup/review/apply window. Eligibility means the game character was fully created before that switch; a pending verification email alone is excluded. Require `rosterWritesFrozen=true` from both public state endpoints and `503 roster_writes_frozen` from roster mutations. The archived dry-run lists every character plus every staff/actively-banned/test/ambiguous exception, explicitly includes approved exception player `542` / `name123`, records an include or exclude decision for every other exception, and binds the exact `reviewToken` plus `asOfMs` to the atomic markers and `launch_subcard_2026:done` seal. Verify one marker per included character, none for exclusions or later characters, keep the freeze through ingress opening, then unfreeze immediately. Preserve older starter-card promises separately.
- [x] Public nginx returns `404` for `/api/admin/*` with and without an admin-token header on every public host; `PORTAL_ADMIN_TOKEN` is unset during normal public operation. Full staff identity/RBAC is deferred, and any bounded maintenance token is loopback-only and removed immediately afterward.
...
- [ ] Resolve the AGPL source-disclosure plan as a separate owner/legal gate. The current release reports `publication_pending` with blank source fields, and the approved launch-preparation slice must not push or advertise the private clean snapshot.

...
- Release source and the enforced launch preset now speak protocol `10132`, while Play production remains `8 / 1.0.7`. Slice 8C produced a clean, signed `9 / 1.0.8` AAB and passed the Play preflight against highest uploaded code `8`, the registered upload certificate, and `server/voidscape-launch.conf`. The artifact remains local and ignored: do not activate the `10132` production server until the Play update is approved and available to players. Play-track availability and physical-device approval remain separate gates.

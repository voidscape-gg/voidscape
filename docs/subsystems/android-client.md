...
2026-07-05 mobile UI parity/QC run: `scripts/build-android.sh` and `scripts/build.sh` passed. Focused chat tabs passed with `ANDROID_SMOKE_AUTH_USER=wbtest ANDROID_SMOKE_AUTH_PASS=voidtest123 ANDROID_SMOKE_AUTH_DB=server/inc/sqlite/preservation.db scripts/android-smoke.sh --no-build --only-auth-chat-tabs --out tmp/mobile-qc-android-chat-tabs-v1`, proving the shared six-band strip selects `CHAT`, `QUEST`, `GLOBAL`, `PRIVATE`, and `ALL` from renderer-reported tab centers. The focused bank smoke passed with the same auth env and `scripts/android-smoke.sh --no-build --only-auth-bank --out tmp/mobile-qc-android-bank-v2`, proving `ANDROID_SMOKE_BANK_OPEN renderer=voidGlass`, mobile bank target sizes (`closeW=54`, `closeH=22`, `loadoutW=38`, `actionH=30`, `menuRowH=28`), search/clear, mobile grid-drag scroll, withdraw, loadout save/load, deposit, deposit-all, logged Close, preset persistence, and fixture restore against `10.0.2.2:43596`. The smoke harness verifies bcrypt-backed fixtures, uses shared-client login geometry/text-length logs for credential entry, seeds a full 192-slot bank fixture for tall Void Glass layouts, and accepts renderer metadata in bank action logs.
...
- [x] Chat tab selection. Smoke reads `ANDROID_SMOKE_CHAT_TAB_LAYOUT`, taps the shared glass tab strip centers for `CHAT`, `QUEST`, `GLOBAL`, `PRIVATE`, and `ALL` with the app-private chat-tab flag enabled, then asserts `ANDROID_SMOKE_CHAT_TAB` reports each selected tab.
...
- [x] Void Glass bank open, scroll, deposit, withdraw, search, presets/loadouts, close. Smoke opens the bank chest fixture, requires `renderer=voidGlass` under the launch config, searches/clears, scrolls with the mobile grid-drag path, withdraws, saves a loadout, deposits, deposit-alls, loads the saved loadout, taps logged Close, verifies persistence after logout, and restores the auth player's DB rows.
...
- [x] Confirm Android back/home/app switcher do not corrupt input state. The focused lifecycle smoke backgrounds the game for 35s, resumes via launcher intent, logs out, and verifies the post-logout keyboard.

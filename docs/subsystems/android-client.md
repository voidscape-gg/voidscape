...
- After bundled cache install, the visible `Play` button writes the selected endpoint to Android app-private `ip.txt` / `port.txt` and starts `GameActivity`. Real devices default to `5.161.114.251:43596`; Android emulators default to `10.0.2.2:43596` so local smoke tests hit the host server without the advanced picker.

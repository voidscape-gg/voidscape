...
- The final 10139 Play candidate uses `targetSdk 35` and `versionCode 12`; versionCode 10 was consumed by the managed-publishing review draft that exposed the account-handoff defect, and versionCode 11 by the held production draft built before the final cache-packaging hygiene fix. Re-read both values from the exact AAB before handoff, then compare the target SDK with Google's upload-day requirement and the version code with every Play track, including drafts.

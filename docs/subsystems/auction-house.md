...
5. **Browse tab**: search by name + 10 category buttons + sortable list (Price up/down, Name, Price-each up/down). Buy a stack or partial qty.
...
- `NewMarketItemTask` — list an item. Validates tradable, price > 0, amount > 0, inventory holds enough, and **queryPlayerAuctionCount < 6**. Removes items from inventory, inserts auction row, falls back to manual rollback on DB failure.
- `BuyMarketItemTask` — purchase. Validates not own listing + has coins + has space. **DB transaction first** (`addExpiredAuction` for seller credit + `setSoldOut`/`updateAuction`), wrapped in `database.atomically(...)`. Inventory/coin mutation only on commit. 5% tax deducted from seller credit.
- `CancelMarketItemTask` — seller cancels; expired-auctions row created so they can collect refund.
- `OpenMarketTask` — re-renders UI; sends raw packet 132 with auction list chunks (200 items each).
- `PlayerCollectItemsTask` — pulls collectible (sold/expired) items into seller inventory/bank.
- `ModeratorDeleteAuctionTask` — admin-only listing nuke (no client-side UI surface).

**Networking**: zero new opcodes. Uses existing `OpcodeIn.INTERFACE_OPTION` (sub-op 10 → AUCTION → sub-sub-op 0=BUY, 1=CREATE, 2=CANCEL, 3=REFRESH, 4=CLOSE, 5=DELETE) handled in `InterfaceOptionHandler.java`. Outbound list payload uses raw packet id 132 from `OpenMarketTask`. Custom outbound `OpcodeOut.SEND_AUCTION_PROGRESS` exists but is currently unused for the marketplace flow.
...
5. **No DB transaction across inventory + auction state**. Inventory is in-memory + saved on autosave/logout. `BuyMarketItemTask` reorders to do DB first, mutate inventory only on commit — eliminates the worst dupe windows but a hard server crash mid-`save()` can still desync.
6. **TOCTOU on stock**: two near-simultaneous buyers can both pre-validate against the same `amount_left`. Voidscape ships with this acceptable for current scale; harden later by re-checking inside the atomically block if contention emerges.
7. **`Market.checkAndRemoveExpiredItems()` runs on the market thread (50ms tick)**. With voidscape's 7-day expiry, it actually fires now (was effectively dead with `TIME_LIMIT = MAX_VALUE`).

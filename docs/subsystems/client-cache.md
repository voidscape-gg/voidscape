...
`CLIENT_VERSION = 10124` in `Config.java` is checked by the server at login. Bump manually when protocol changes; mismatch -> rejection. Keep the server `client_version` in each active `.conf` aligned with this value.
...
2. **Silent version mismatch.** `CLIENT_VERSION = 10124` is not auto-bumped. If you change protocol but forget to bump, server may accept the connection but corrupt state. Check server logs for version checks.

...
`overmatch_combat: true` selects the optional opposed-margin ruleset documented in `docs/design/overmatch-combat-formula.md`. Melee, ranged, and successful damaging magic casts use contested integer dice and disjoint glance/solid/critical damage bands; melee criticals and overwhelming defensive wins against player attackers create one target-bound Edge stack for the next melee swing. The flag defaults off and is mutually exclusive with `openrsc_classic_combat_baseline`, `osrs_combat_melee`, and `osrs_combat_ranged`; an invalid combination stops server startup. Overmatch preserves Void weapon target bonuses, NPC damage floors, prayers, combat scripts, XP policy, Protect from Magic, and cape eligibility. The Ranged cape's second arrow is resolved as a second Overmatch contest. No-Magic stake duels remain ordinary/unverified while this experimental ruleset is enabled because the shipped proof replayer only commits to the normal Voidscape formula.

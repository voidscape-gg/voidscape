...
`overmatch_combat: true` selects the official launch opposed-margin ruleset documented in `docs/design/overmatch-combat-formula.md`. Melee, ranged, and successful damaging magic casts use contested integer dice and disjoint glance/solid/critical damage bands; melee criticals and overwhelming defensive wins against player attackers create one target-bound Edge stack for the next melee swing. The safe Java/default preset value remains off, while the reviewed launch contract requires it on. It is mutually exclusive with `openrsc_classic_combat_baseline`, `osrs_combat_melee`, and `osrs_combat_ranged`; an invalid combination stops server startup. Overmatch preserves Void weapon target bonuses, NPC damage floors, prayers, combat scripts, XP policy, Protect from Magic, and cape eligibility. The Ranged cape's second arrow is resolved as a second Overmatch contest. No-Magic stake duels remain ordinary/unverified because the shipped proof replayer only commits to the normal Voidscape formula.
...
A third ruleset, `--rules overmatch`, models the proposed opposed-margin combat
formula (design stage only, not implemented on any server); its spec lives in
`docs/design/overmatch-combat-formula.md`.

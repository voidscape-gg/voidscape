...
Launch scope decision (2026-07-12): the iPhone web client is deferred from the July 18 launch and must not be advertised as launch-ready. Physical iPhone Safari/Home Screen QA was not completed, and neither the owner decision nor existing local/Simulator/hosted evidence converts that gate into a pass. The shared TeaVM/browser work remains intact for post-launch testing; keeping `/play/` available for supported browser use is not an iPhone support claim.

...
Account creation/recovery on `/play/` is portal-first. The shared Java client emits `voidscape:account` and `voidscape:recovery` from `osConfig`; `index.html` resolves those sentinels through query/global portal settings before opening the account manager. Release `/play/` routes Create Account directly to the rules-gated launch registration surface rather than the legacy sign-in hash or packet registration. Native Android follows the same portal-first registration contract through `/portal?auth=register`; desktop Java alone retains in-client packet registration.

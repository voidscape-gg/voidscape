...
The latest `/play/` package is deployed on `https://voidscape.gg/play/` as client version `10120`, generated at `2026-06-27T15:12:52Z`, with `fileCount: 421`, asset token `cache-356134644e1c926b5ee2`, manifest SHA-256 `004a449a721d370e5107259eefbfbba79173c2a7d9efcc7858e93479a72353f7`, and `voidscape-web-client.js` SHA-256 `95654226d5df9cf78a9d7575bca61fbcd31204f7ae84ece2b71e52bde1b4f0af`. The live backup before this deploy is `/opt/voidscape/backups/play-iphone-inventory-pre-20260627T151304Z.tgz`; hosted static/deep verification passed at `tmp/web-teavm-deployment-verify-iphone-inventory-fix` with all 421 manifest files verified and direct live manifest/JS hashes matching the local package. This package keeps clean `/play/` on `wss://voidscape.gg/play/ws/`, preserves the shared Java renderer/protocol, carries the prelaunch mobile existing-user login ergonomics, and fixes iPhone Safari inventory taps by allowing small finger jitter inside open panels to still produce a release click unless a real scroll gesture was detected. The shared Java client also keeps the inventory menu/action executor active while the Voidscape HUD marks the pointer inside the inventory panel. Authenticated hosted smoke was not rerun for this inventory deploy because the available default/local QA credentials were not valid in this session; local no-login controls smoke and hosted static/deep deployment verification are current.

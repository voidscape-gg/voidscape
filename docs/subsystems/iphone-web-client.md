...
The latest `/play/` package is deployed on `https://voidscape.gg/play/` as client version `10120`, generated at `2026-06-26T00:58:07Z`, with `fileCount: 422`, asset token `cache-5f34d4087bf307f39cb6`, manifest SHA-256 `6444b9af7171fec5ba530b7b2ec4fd835d6bf52a56fa086c52d4331209f88ecd`, `voidscape-web-client.js` SHA-256 `49363d8080708f29d143d44b0471c20efc446990208b451a24f076a50aeadada`, and `Cache/video/Authentic_Sprites.orsc` SHA-256 `29cb58b5b3e4d2876fe0064bfdc416f395f97c7b8a27e80443207e3fe9ddddca`. Static/deep hosted verification passed at `tmp/web-teavm-deployment-verify-10120-rightclick-menu-select-v1`; the live backup before this deploy is `/opt/voidscape/backups/deploy-10120-rightclick-menu-select-pre-20260626T005830Z`. This package keeps clean `/play/` on `wss://voidscape.gg/play/ws/`, keeps desktop mode at the native non-stretched `512x346` framebuffer/CSS canvas, retains the desktop navigation-key text guard, keeps the tiny-desktop overlay safe area for custom bank, legacy bank, and `::onlinelist`, and makes Android-style mobile long-press/right-click use CSS-pixel movement tolerance so normal phone thumb wobble no longer cancels the context menu before it fires. It also fixes the physical-phone context follow-up: the top mouse menu opens near the long-press point, survives release, and taps inside the menu activate a normal menu row instead of doing nothing. Local controls proof passed at `tmp/web-teavm-controls-rightclick-menu-select-v1`; focused local proof passed at `tmp/web-teavm-iphone-context-menu-local-v6`; live focused proof passed at `tmp/web-teavm-iphone-context-menu-live-v2` with `anchoredNearPress: true` and `selectEvents: ["m,256,506"]`. The last direct hosted key-buffer proof remains `tmp/web-teavm-hosted-nav-keys-focused-v1`, proving PageUp/PageDown/arrow keydowns no longer mutate Java `chatMessageInput` or `inputTextCurrent` even when a browser follows with legacy printable keypress codes such as `%`; a standalone printable `%` keypress still queues text. The latest broad hosted desktop smoke remains blocked by the live `test` account already being logged in, so `tmp/web-teavm-hosted-desktop-arrow-legacy-percent-v1` remains the latest full logged-in desktop mouse/terrain proof from the prior package. The web package stamps cache-resource URLs from packaged `Cache/` contents, so immutable browser-cached assets such as `Authentic_Sprites.orsc` refresh when sprites/projectiles change. Normal local menu/contact-sheet visual QA passed earlier at `tmp/web-teavm-visual-polish-pass-v5-final-normal`. A local-only forced-selector visual pass at `tmp/web-teavm-combat-selector-forced-visual-v4` proved Java-side combat-style selector placement now uses the lower chat area instead of the top status lane on mobile and that phone-landscape loose chat shifts beside it; the debug forcing was reverted before the deployed build.
...
Account creation/recovery on `/play/` is portal-first. The shared Java client emits `voidscape:account` and `voidscape:recovery` from `osConfig`; `index.html` resolves those sentinels through query/global portal settings before opening the account manager. Release `/play/` should therefore route Create Account to the same launch-signup account manager as desktop and Android rather than packet registration.

It is not production-shippable until these external gates pass:
...
- The real canvas chat and canvas chat tabs own normal chat display/input on iPhone. `WebClientPort` publishes `ui.chatPanelHidden`, `ui.chatAccessMode`, and `ui.phoneLandscapeLooseChat`. Phone portrait keeps the framed Voidscape chat history, while phone landscape suppresses the large framed history box and draws recent chat as loose OpenRSC-style text above the shared bottom chat tabs. The compact browser chat-icon helper/tray is hidden in normal Android-parity mode so it does not duplicate the shared chat HUD. The browser `Aa` keyboard helper and secondary/context-action helper remain Safari affordances that feed the shared input path; long-press still queues secondary clicks.
...
- Controls smoke proves the tray is hidden by default, opens from the `Chat` button, exposes all five shortcuts with touch-sized targets, queues `c,<chat>`, opens keyboard for `compose`, closes after each shortcut, and lets browser Back close the tray without queuing shared Back.

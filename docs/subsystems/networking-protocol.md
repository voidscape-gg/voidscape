...
**Protocol version** — `Client_Base/src/orsc/Config.java` `CLIENT_VERSION = 10112`. Server's `client_version` config key (e.g. `10112`) is checked at login. Mismatch → reject when the preset enforces custom client versions. Bump manually when protocol changes.
...
- `10112`: DM King Void Arena challenge. Adds client-visible DM King NPC definitions for the lobby challenge NPC (`862`) and dynamic fight NPC (`863`) plus the lobby spawn; the visible definition deliberately reuses the Death Match Arena Void Knight appearance. Also renders the existing custom NPC item-bubble update used for DM King's potion sip and consumes hidden `@vsarena@dmking|wins|losses` metadata for the DM King `Challenge` row W/L label. No packet shape, opcode, enum ordinal, or cache archive changed.

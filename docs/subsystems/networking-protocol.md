...
**Protocol version** — `Client_Base/src/orsc/Config.java` `CLIENT_VERSION = 10123`. Server's `client_version` config key (e.g. `10123`) is checked at login. Mismatch -> reject when the preset enforces custom client versions. Bump manually when protocol changes, client-visible cache files change, or a client binary update must be forced.

...
**Protocol version** — `Client_Base/src/orsc/Config.java` `CLIENT_VERSION = 10132`. Server's `client_version` config key (e.g. `10132`) is checked at login. Mismatch -> reject when the preset enforces custom client versions. Bump manually when protocol changes, client-visible cache files change, or a client binary update must be forced. The coordinated Slice 8 source bump activates the launch cracker-HUD capability for shared PC, native Android, and TeaVM builds.

Voidscape custom-client packet notes:
- `10132`: finite launch-cracker state reuses senderless `SEND_SERVER_MESSAGE` / `MessageType.QUEST` metadata as `@vscrackercampaign@v1|remaining`. `remaining` is decimal `0..1000000`; `0` hides the HUD. The server sends a snapshot after game settings on normal login and reconnect/resume, broadcasts owner-set and settled award changes, and gates the envelope to custom clients at or above this version. The shared client consumes recognized envelopes before chat display; malformed version/field count/integer/range fails closed to hidden. Failed/uncertain awards and uncertain owner sets publish hidden until an authoritative reload; a confirmed owner-set rollback republishes its known previous count, while award outcomes that do not change the pool publish nothing. No opcode, enum ordinal, or packet payload shape changed. The enforced launch preset now rejects older custom clients because they would expose hidden metadata and lack the HUD.
- `10131`: client-visible Void gear identity cohort. Item `1596` displays as Void Sceptre, all four descriptions state their exact PvE roles, Scimitar tradability is synchronized, and the dormant items-on-death preview mirrors the server's death-only Void gear priority. The same unreleased RC cohort synchronizes the Christmas-cracker reel with Bunny ears `1156` and Scythe `1289`; black party hat `1582` remains valid only when both server and client use `custom_sprites=true`. No opcode, payload, or packet shape changed.
...
- `10128`: experimental four-stage Dungeon/Wyrm cache cohort. It is retained only as version history; `10130` removes the fourth floor and active Wyrm path from the release runtime.
- `10127`: Void Rift scenery id `1306` loads the original `voidriftportal.ob3` structure from `models.orsc`, with shared-client flat lighting and restrained inner energy replacing its procedural ground disc. No opcode, payload, or packet-shape change.

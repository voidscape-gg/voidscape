...
**Protocol version** — `Client_Base/src/orsc/Config.java` `CLIENT_VERSION = 10139`. Server's `client_version` config key (e.g. `10139`) is checked at login. Mismatch -> reject when the preset enforces custom client versions. Bump manually when protocol changes, client-visible cache files change, or a client binary update must be forced.

Voidscape custom-client packet notes:
- `10139`: PK Catching Simulator UI cohort. The server sends bounded
  `@vspkcatch@v1|...` chooser, session/HUD, result, and atomic leaderboard-transfer metadata as
  senderless Quest messages through existing `SEND_SERVER_MESSAGE`; the shared client consumes the
  namespace before chat display. Difficulty choice still uses the ordinary three-option menu and
  packet-116 reply, including `-1` cancellation. Legacy result/highscore boxes remain the fallback,
  and the existing 14-field HUD mirrors the Easy / Trainer destination for older compatible clients.
  Updated clients also accept the additive five-field `guide|session|state|x|y` record: `destination`
  carries a stable server-authoritative world tile, while `attack` and `hidden` require `-1|-1`.
  Unknown records inside the reserved namespace are consumed without chat leakage, so older 10139
  clients safely ignore `guide`. No opcode, enum ordinal, cohort, or packet shape changed.
- `10138`: existing game-setting byte index 35 expands from a boolean floating-name toggle to
  the clamped remote-player label modes `0` Names + titles, `1` Names only, and `2` Hide names +
  titles. The setting keeps the same opcode, byte position, and packet length. The server stores
  the integer in a new player-cache key, migrates the old boolean on read, and sends the legacy
  boolean meaning to pre-10138 clients. Official presets enforce the cohort because a stale 10137
  renderer cannot honor all three states. No title-appearance field moved: the honorific tail
  remains gated at 10137.
- `10137`: title-ceremony and honorific cohort. Custom player-appearance records append
  the active honorific string and its one-byte tier after the existing modern-hair byte;
  the suffix title string and tier keep their prior positions. The live-player decoder,
  null-player alignment branch, and voidbot consume the same tail. Prefix honorifics and
  suffix epithets can render together, while `~vstitleaward~` metadata skins the existing
  two-option menu transport as a glass unlock ceremony. The title catalog and Court keep
  using readable `~vstitle~` menu rows. No opcode was added or reordered, but the appended
  appearance fields require the cohort bump.
- `10136`: combat-level initiative proof cohort. Canonical duel-proof context v3
  (`VSDPCTX3`) adds each participant's positive displayed/base combat level. Both
  clients attest their own and the visible opponent's committed levels before adding
  entropy; the server re-encodes the same context before combat. Draw one is still
  consumed from the committed stream to preserve every later RNG boundary, but it
  selects the starter only when the two displayed levels are equal. Otherwise the
  lower-level participant deterministically starts. Ineligible stake duels use the
  same lower-level rule with an ordinary canonical-ID coin only on exact ties. The
  witness-v2 wrapper, hidden `@vsduelproof@v1` / `@vsduel@v2` transports, opcode,
  packet shape, database schema, and No Magic eligibility are unchanged.
- `10135`: legal-retreat and proof-start acceptance cohort. The existing `@vsduelproof@v1|abort|<proof-id>|<reason>` envelope adds the bounded `retreat` reason. Only that reason retires the matching seed-free client lock anchor because an allowed retreat has no terminal receipt; timeout, state, database, and generic cancellation failures retain their omission alarm. Server-side, legal retreat becomes an intentional `COMBAT -> ABORTED / RETREAT` outcome with no receipt, witness, winner, or stake transfer. Adjacent proof duelists also overlap approach with the handshake and atomically persist the existing `LOCKED` then `COMBAT` transitions after both exact lock ACKs, saving one callback tick without changing any commit/reveal control or durability guarantee. No opcode or packet shape was added.
- `10134`: terminal melee-proof verification cohort. The pre-combat control transport deliberately remains `@vsduelproof@v1|...` even though its canonical context is version 2: bounded lowercase-hex `context` chunks (160 raw bytes per chunk) arrive before `commit`, and the client verifies the context hash/encoding and proof id and attests the locally visible subset of its own starting state, displayed opponent identity, and both displayed stake offers before generating or contributing secure entropy. The server's context-bound commitment was already durable before those controls; both client commitments still become durable before either reveal, and both reveals/final lock/ACKs are durable before guarded `LOCKED -> COMBAT`. PC/Android use `SecureRandom`; TeaVM uses browser `Crypto.getRandomValues` without a weak fallback. Controls travel as hidden senderless Quest messages under the `@vsduelproof@` prefix, and replies still reuse `INTERFACE_OPTIONS` wire opcode `199`, sub-option `19`. Authorized `::duel` detail responses use `@vsduel@v2`: a verified receipt adds one bounded `proofmeta` record followed by lowercase-hex `proofdata` chunks (192 raw bytes per server chunk, 256 KiB witness maximum) before the existing `end`; an ineligible receipt sends `proofmeta|0`. While a duel is active, the server instead sends one bounded `blocked|duel_active` v2 control so the client clears its pending request, leaves the journal closed, and tells the player to retry after settlement. The shared PC/Android/TeaVM client assembles canonical witness v2 only for that request, verifies its transport hash and encoding, opens all three locked seeds and commitments, derives the committed starter, independently replays every melee draw plus the complete HP/recoil terminal, and cross-checks both stake lists, receipt identity/time/result, and the requester's private swing rows. It retains up to 16 seed-free lock anchors for the login session; a bare latest-receipt request must select history row zero and match the newest retained anchor exactly, while matching anchors also strengthen explicit recent-history reads. Login/logout/reconnect wipes those anchors. The client wipes raw witness buffers after verification; only `VERIFIED` / `FAILED` / `UNAVAILABLE` and the short proof id enter the UI model. The witness necessarily contains both participants' formula inputs, dynamic styles/prayer tiers, and seeds, so wiping is hygiene rather than secrecy from a modified client; the ordinary modal still has no opponent-swing model. No opcode or packet shape was added, and no website/public log or backfill is part of this protocol.
- `10133`: historical foundation cohort for the fail-closed classic-formula No Magic commit/reveal/lock handshake. Server controls used hidden senderless Quest messages with the `@vsduelproof@v1|...` prefix (`commit`, `reveal`, `lock`, or `abort`); client replies reused custom `INTERFACE_OPTIONS` wire opcode `199`, sub-option `19`, as one printable-ASCII string of at most 192 characters with exactly one final line feed (`commit`, `reveal`, `ack`, or `fail`). The server durably stored its context-bound commitment before challenging either client, both client commitments before asking for either seed, both validated private reveals before the seed-free final lock, and required both exact lock ACKs before guarded combat entry. Cohort `10134` retains that envelope and extends it with canonical context-v2 delivery/attestation and terminal witness-v2 verification. Magic-allowed duels and worlds using the OSRS melee formula never enter the proof handshake.
- `10132`: `::duel` uses the existing inbound `COMMAND` packet and returns a versioned `@vsduel@v1|...` control envelope as senderless `MessageType.QUEST` traffic through existing `SEND_SERVER_MESSAGE`; no opcode or packet shape was added. The shared client accepts only a complete, internally consistent `begin..end` sequence for one request id before opening the journal, while older clients receive a readable one-line summary. A detail lookup is authorized by a participant join, includes both accepted stake offers, and selects swings only where `actor_player_id` is the requester, so an opponent's hits and combat modes are never sent to that player. This is a private server-authored combat audit trail, not a seed, commit/reveal, or replay proof of RNG.
- `10131`: client-visible Void gear identity cohort. Item `1596` displays as Void Sceptre, all four descriptions state their exact PvE roles, Scimitar tradability is synchronized, and the dormant items-on-death preview mirrors the server's death-only Void gear priority. No opcode, payload, or packet shape changed.
- Paperdoll V2 evaluation reuses the existing `PLAYER_APPEARANCE_CHANGE` opcode,
  payload width, and `hairStyle` tail. Its selectable-head decoder interprets
  the existing head byte as unsigned before adding one, so appearance IDs
  `129..256` no longer wrap negative; body decoding is unchanged. This changes
  one byte's server-side semantics without adding a field or packet-shape gate.
- `10129`: self-opened Christmas crackers send a versioned `@vscracker@v1|rollId|category|itemId|seed` control envelope through the existing `SEND_SERVER_MESSAGE` packet after the reward is committed. The shared PC/Android/web client consumes the envelope without adding it to chat and runs a visual-only deterministic reel; the ordinary result message remains as accessible fallback/history. No opcode or packet shape changed, but stale clients would expose the control text and miss the reel, so the client version is bumped.
- `10130`: the release Void Dungeon replaces the retired fourth floor and Void Wyrm with a polished three-stage landscape, baked minimap boundaries, plane `1..3` world maps, and floor-aware map controls. No opcode, payload, or packet shape changed; the bump rejects clients with stale dungeon terrain or maps.
...
- `10117`: custom clients interpret the existing player appearance overhead byte as a bitmask. Low bits `0..2` preserve the skull/bounty-skull meanings, bit `4` means Protect from Missiles is active, and bit `8` means Protect from Magic is active. The packet length and opcode are unchanged; clients below `10117` still receive the old skull-only value.
- `10118`: Fire Blast uses the existing projectile update with projectile type `9`, backed by `Authentic_Sprites.orsc` archive entry `3169` and client `SPRITE_PROJECTILE_COUNT = 10`. The client renders that projectile type at 37x37 scene size instead of the standard 32x32 so only Fire Blast gets the larger missile presentation. No packet shape or opcode changed, but stale clients must be rejected because older `10117` clients only load projectile types `0..8`.
- `10119`: Fire Blast projectile slot `9` now stores ARGB alpha in `Authentic_Sprites.orsc` archive entry `3169`, and the PC/shared client routes only that projectile through a projectile alpha draw path at 56x56 scene size. `tools/voidscim-art/pack.py --preserve-alpha` is opt-in; legacy sprites still pack as hard-mask `0x00RRGGBB`. No packet shape or opcode changed, but stale `10118` clients lack the alpha draw path and expected size.
- `10120`: Wind Blast, Water Blast, and Earth Blast now use matching ARGB alpha projectile types `10`, `11`, and `12`, backed by `Authentic_Sprites.orsc` archive entries `3170`, `3171`, and `3172`; Fire Blast remains type `9` / entry `3169`. The shared client loads `SPRITE_PROJECTILE_COUNT = 13`, routes the Blast family through the alpha draw path, and renders those projectiles at 50x50 scene size. No packet shape or opcode changed, but stale `10119` clients only know projectile types `0..9`.
- `10121`: `SEND_BANK_UPDATE` (custom wire opcode `249`) widens its slot field from one byte to a short for custom clients `>= 10121` — the byte wrapped mod 256, corrupting per-slot bank updates past slot 255 (VS-008). Payload is now `short slot`, `short catalogId`, `int amount`; clients below `10121` keep receiving the one-byte slot (and 10009/10010 keep their whole-bank resend quickfix in `ActionSender.updateBankItem`). Packet-shape change — requires the version bump; voidbot's decoder follows its negotiated `VOIDBOT_CLIENT_VERSION`.
- `10123`: custom `SEND_UPDATE_PLAYERS` appearance type `5` appends a one-byte player-title tier after the active player-title string for clients `>= 10123`; modern hair remains after this new byte. The client uses the tier only for overhead title color and no longer injects active titles into public chat text. Packet-shape change — requires the version bump.
- `10124`: custom public-chat, `SEND_SERVER_MESSAGE`, and received-private-message icon ints reserve bit `0x40000000` to mark an active Void subscriber sender. The client masks the bit before crown drawing and renders only the sender display name in magenta, then resets the rest of the chat line to the message type's normal color. No opcode or payload length changed, but stale clients would treat the bit as part of the crown id, so the client version is bumped.
- `10125`: custom `PLAYER_APPEARANCE_CHANGE` (wire opcode `235`) appends two country-code bytes after `hairStyle` and before the optional referral string. `0x00 0x00` means None; otherwise the bytes are uppercase ISO 3166-1 alpha-2 characters validated against the server whitelist. `SEND_UNLOCKED_APPEARANCES` (wire opcode `250`) appends the same two-byte current selection after its bit-packed unlock masks so Makeover Mage re-entry preselects the stored flag. The country list is duplicated client/server, but the wire sends the code rather than a list index. Packet-shape change — requires the version bump; older packet shapes remain accepted and preserve existing country cache state.
- `10126`: client-visible cache/world-map refresh for the two-floor Void Dungeon redesign: `Custom_Landscape.orsc` now contains the expanded dungeon floor mask and the world map labels/POIs include the surface Void Dungeon entrance. No opcode, payload, or packet-shape change.
- `10127`: Void Rift scenery id `1306` now loads the original `voidriftportal.ob3` structure from `models.orsc`, with shared-client flat lighting and restrained inner energy replacing its procedural ground disc. No opcode, payload, or packet-shape change; the version bump rejects clients missing the new cache entry and renderer path.
- `10128`: The Void Dungeon uses a regenerated four-stage `Custom_Landscape.orsc`, and the custom Void Wyrm NPC uses an 18-frame block in `Authentic_Sprites.orsc` entries `1917..1934`. No opcode, payload, or packet-shape changed; the bump rejects clients missing the new terrain and boss art.

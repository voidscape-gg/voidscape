...
Currently **empty** (only `.gitsave` placeholder). Intended for per-player appearance data but not used. Appearance lives in the `players` table columns (`male`, `haircolour`, `topcolour`, etc.). Custom appearance overrides could land in `player_cache`.

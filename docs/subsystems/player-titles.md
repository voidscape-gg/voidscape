...
- `server/src/com/openrsc/server/content/PlayerTitle.java` is the title registry. It owns the 54-entry catalog, tier/lifecycle metadata, cache keys, lookup helpers, and automatic unlock checks.
...
- `::titles unlocked|renown|feat|unique [page]` opens a filtered catalog page. Rows show title, tier, and state; selecting a row opens requirement/progress/lifecycle/holder details.
...
The custom PC client recognizes the title menu payloads and renders them as centered
glass modals with All/Unlocked/Renown/Feat/Unique tabs. The modals still send ordinary
menu replies, so the server remains authoritative and no title-catalog opcode is
required.
...
- fishing/cooking/bone burying/mining/high-alchemy/gnomeball counters feed Lobster Baron, Unburnt, Bone Collector, Coal-Hearted, Goldspinner, and Gnome-Baller.

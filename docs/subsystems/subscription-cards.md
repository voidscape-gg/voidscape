...
The release campaign is a one-time final-roster cutover, not a character-creation or login reward. The public landing and signup flow stay live during launch preparation. About ten minutes before ingress opens, operators use the tested roster-freeze switch so the reviewed native-account cutover can snapshot every current save, require an explicit decision for every staff, banned, invalid, or obvious test identity, and seed one global marker named `launch_subcard_2026:<playerId>` per included character. Value `1` is available and value `2` is claimed. The immutable player id makes the entitlement character-specific without requiring a portal link, while keeping the marker as an audit tombstone if the portal later deletes that save.

The same SQLite transaction writes every missing campaign marker and `launch_subcard_2026:done = 1`. Once that seal exists, rerunning account backfill may repair ownership records but can never add launch cards for characters created after the snapshot. Deleting a launch character forfeits an unclaimed card; a replacement character does not inherit it. Eligibility means the game character was fully created before `PORTAL_ROSTER_WRITES_FROZEN=1` took effect; requesting or receiving a verification email without completing verification does not qualify. Use `scripts/manage-portal-roster-freeze.sh freeze` only for the approximately ten-minute backup/review/apply window, keep it frozen until ingress opens, then unfreeze immediately. The owner has explicitly approved exception player `542` / `name123` for inclusion in the launch-card cohort. Its guarded offline promotion and exact rank/link/hash evidence happen before the freeze, while real OWNER authentication is proven only during the post-apply private boot.

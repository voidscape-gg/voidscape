...
write_manifest() {
	...
	{
		printf 'version=10125-%s\n' "$(date -u +%Y%m%d%H%M%S)"
		printf 'clientVersion=10125\n'
		printf 'baseUrl=%s/\n' "$BASE_URL"
		local index=1
		...
		if [[ -n "$launcher_jar" ]]; then
			...
			printf 'launcher.version=10125-smoke\n'
		fi
...
check "fresh install reports client version" log_has "SYNC_CLIENT_VERSION 10125"

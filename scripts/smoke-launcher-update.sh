...
write_manifest() {
	...
	{
		printf 'version=10122-%s\n' "$(date -u +%Y%m%d%H%M%S)"
		printf 'clientVersion=10122\n'
		printf 'baseUrl=%s/\n' "$BASE_URL"
		local index=1
		...
		if [[ -n "$launcher_jar" ]]; then
			...
			printf 'launcher.version=10122-smoke\n'
		fi
...
check "fresh install reports client version" log_has "SYNC_CLIENT_VERSION 10122"

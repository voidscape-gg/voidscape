...
TEST_SOURCE="$REPO_ROOT/server/test/java/com/openrsc/server/ServerConfigurationExplicitConfigTest.java"
TEST_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/voidscape-server-explicit-config.XXXXXX")"
...
(
	cd "$FIXTURE"
	"$JAVA_BIN" -cp "$SERVER_JAR:$CLASSES" com.openrsc.server.ServerConfigurationExplicitConfigTest
)
python3 "$SCRIPT_DIR/test_server_explicit_config.py"

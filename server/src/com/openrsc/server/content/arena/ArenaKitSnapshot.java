package com.openrsc.server.content.arena;
...
public final class ArenaKitSnapshot {
	...
	public static String capture(Player player) {
		try {
			...
			throw new IllegalStateException("Unable to capture arena kit snapshot", e);
		}
	}

	public static void restore(Player player, String encoded) {
		try {
			DataInputStream in = new DataInputStream(new ByteArrayInputStream(Base64.getDecoder().decode(encoded)));
			int version = in.readInt();
			if (version != VERSION) {
				throw new IllegalArgumentException("Unsupported arena kit snapshot version " + version);
			}
			...
			throw new IllegalStateException("Unable to restore arena kit snapshot", e);
		}
	}

	public static void clearContainers(Player player) {
		synchronized (player.getCarriedItems().getInventory().getItems()) {

    ...
    */ 
	...
	public void processNextMovement() {
		...
		if (!PathValidation.checkAdjacent(mob, new Point(startX, startY), new Point(destX, destY))) {
			// voidscape: auto-open simple closed doors/gates that block a
			// route, then retry the same step next tick.
			if (mob.isPlayer() && AutoOpenRouteObstacle.tryOpenBetween(
				(Player) mob, new Point(startX, startY), new Point(destX, destY))) {
				if (mob.getX() != destX || mob.getY() != destY) {

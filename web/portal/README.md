...
- Google reservation handoff that asks for a 4-20 character game password, converts the reserved roster card into a real linked game save, then reveals the launcher download
...
To let the Characters screen load real saved character state from a local game SQLite database, start the server with:

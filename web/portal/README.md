...
`PORTAL_LAUNCH_FREE_CARD_HOURS=0` is the launch policy, but the one-shot roster cutover—not the countdown—is the launch-card authority. A character qualifies only if it was fully created before the brief launch roster freeze; a pending verification email alone does not qualify. A positive runtime window must not be used for this release.

Set `PORTAL_WEB_CLIENT_URL=https://voidscape.gg/play/?phone=1` if the browser client moves. The July 18 launch supports Web, Desktop, Android, and iPhone Safari through the same browser client. iPhone support may be less polished; physical Safari/Home Screen QA is owner-waived/not passed, and no native iOS/App Store application is claimed. Leave `PORTAL_ANDROID_PLAY_URL` unset while the Play release is still under review. After code `9 / 1.0.8` is publicly available, set it to `https://play.google.com/store/apps/details?id=com.voidscape.gg`; Google Play and the configured release-signed direct APK are both first-class Android choices. The public page keeps the clickable funnel on reserving the account name until the world opens. Once `PORTAL_LAUNCH_AT` is in the past, the same landing switches into the launch-open surface: desktop launcher download, web/iPhone browser client, both Android choices, live build proof, and account-management CTAs render from `/api/public` download metadata.
...
The native-account backfill is a cutover operation, not a recurring service. It creates synthetic portal ownership records for real game accounts present at the cutover, preserves active subscription time, keeps older account-level starter promises intact, and seeds one `launch_subcard_2026:<playerId>` entitlement for every included final-roster character. Keep signup live during preparation. Before the freeze, keep the game inactive/disabled and use `scripts/promote-owner-sqlite.sh` to dry-run and then apply the exact player `542` / account `225` / `name123` offline OWNER promotion, archiving its rank/link/integrity and backup/restored-copy hashes; this does not yet prove authentication. About ten minutes before opening, run `scripts/manage-portal-roster-freeze.sh freeze`; it fails closed unless the game unit is inactive and disabled/masked and configured production game ports have no listeners. Require `/api/public` and `/api/health` to report `rosterWritesFrozen=true` and roster mutations to return `503 roster_writes_frozen`. Take verified backups of both the game DB and `$PORTAL_DATA_DIR/dev-store.json`, then review the dry-run before applying:
...
If the systemd unit is installed, put the reviewed token and exception decisions in its private `portal.env`, invoke it once with `systemctl start voidscape-native-portal-backfill.service`, then remove those one-use values. Do not enable the unit, and do not install a timer. Re-run `--dry-run` afterward and require zero pending changes plus a present cutover seal. Keep the game stopped through backup and apply; boot it privately behind closed ingress only afterward. During that private boot, log in as `name123`, run read-only `::loadbots status` without starting bots to prove OWNER command access, then prove save/logout/relogin. At the announced time, open ingress and immediately run `scripts/manage-portal-roster-freeze.sh unfreeze`.
...
- `PORTAL_ROSTER_WRITES_FROZEN=1` is the bounded launch-card roster switch managed by `scripts/manage-portal-roster-freeze.sh`; normal production is `0`. Frozen public state reports `rosterWritesFrozen=true`, roster mutations return `503 roster_writes_frozen`, and landing, resend, login, and security remain available.
- `PORTAL_WEB_CLIENT_URL=https://voidscape.gg/play/?phone=1` controls the release web-client URL used by API metadata and post-launch surfaces; the portal normalizes bare `/play/` values to include `?phone=1` for the existing phone-shell route. This route is the launch-supported iPhone Safari option, with physical Safari/Home Screen QA owner-waived/not passed and no native/App Store claim. The prelaunch landing does not link play/download actions.
- `PORTAL_ANDROID_PLAY_URL=https://play.google.com/store/apps/details?id=com.voidscape.gg` enables the Google Play Android choice. Leave it unset until Play has approved the production release: unset means no Play row and preserves the existing APK-only UI. The server accepts only the HTTPS `play.google.com` details page for package `com.voidscape.gg`, strips locale/campaign parameters to the canonical URL, and fails startup for any other host, path, scheme, duplicate package id, or package name. When enabled, Google Play and the configured signed direct APK are both first-class Android choices.
...
- Platform-support proof copy for Web, Desktop, Android, and browser-based iPhone Safari while keeping the prelaunch CTA focused on reserving a name and starter card; physical Safari/Home Screen QA remains owner-waived/not passed
